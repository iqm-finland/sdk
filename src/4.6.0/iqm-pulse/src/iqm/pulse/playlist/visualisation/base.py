...
def _numpy_to_builtin_types(data: list[tuple[str, Any]]) -> dict[str, Any]:
...
def _get_waveform(wave: Waveform, scale: float, wave_q: Waveform | None = None, scale_q: float | None = None) -> str:
    ...
    def fig_to_base64(fig: plt.Figure) -> bytes:
        img = io.BytesIO()
...
def inspect_playlist(playlist: Playlist, segments: Sequence[int] = (0,)) -> str:
    ...
    vis_js_style = Path(path / "static" / "vis-timeline-graph2d.min.css").read_text(encoding="utf-8")
    vis_js_script = Path(path / "static" / "vis-timeline-graph2d.min.js").read_text(encoding="utf-8")
    moment_js_script = Path(path / "static" / "moment.min.js").read_text(encoding="utf-8")
