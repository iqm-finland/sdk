...
## Version 35.0.2 (2026-08-26)

### Features

- Prepare package for `4.6.2` release. No functional changes.

## Version 35.0.1 (2026-08-05)

### Features

- Prepare package for `4.6.1` release. No functional changes.

## Version 35.0.0 (2026-07-07)

### Breaking changes

- `IQMClient.__init__` no longer accepts deprecated URL formats:
  - URLs containing the quantum computer name (e.g.,
    `https://resonance.meetiqm.com/garnet`) — use server base URL and pass
    the quantum computer name via `quantum_computer` parameter
  - URLs with the `:timeslot` suffix — use `use_timeslot=True` parameter
    when submitting a job instead
  - `https://cocos.resonance.meetiqm.com` URLs — use `https://resonance.iqm.tech`
    instead
- `IQMClient` now raises an error for insecure HTTP requests to remote
  hosts. Use `https://` URLs instead.
...
- Improved security in _IQMServerClient by stripping authorization
  headers on plain HTTP calls and blocking automatic redirects for
  mutating requests. The client will now actively warn or raise errors
  instead of proceeding silently.
- Add `quantum_computer_name` property to `IQMClient` and use it to initialize
    `IQMBackend.name`.
- Allow IQMClient init parameters `iqm_server_url` and `quantum_computer` to
  be given as environment variables IQM_SERVER_URL and IQM_QUANTUM_COMPUTER.
- Add optional dependency groups with pinned versions to `iqm-client`:
  ...
  - `cirq-pin-iqm`: includes `cirq` extra dependencies with inter-IQM packages
    pinned to co-released versions.
  - `cirq-pin-all`: like `cirq-pin-iqm`, but additionally pins all transitive
    third-party dependencies.
  - `cli-pin-iqm`: includes `cli` extra dependencies with inter-IQM packages
    pinned to co-released versions.
  - `cli-pin-all`: like `cli-pin-iqm`, but additionally pins all transitive
    third-party dependencies.
  - `qiskit-pin-iqm`: includes `qiskit` extra dependencies with inter-IQM
    packages pinned to co-released versions.
  - `qiskit-pin-all`: like `qiskit-pin-iqm`, but additionally pins all
    transitive third-party dependencies.
- IQMClient init parameters `iqm_server_url` and `quantum_computer` have
  priority over environment variables IQM_SERVER_URL and IQM_QUANTUM_COMPUTER.
- Added the `iqm.qrisp_iqm` package, which allows executing Qrisp circuits on IQM
  quantum computers. Requires the `iqm-client[qrisp]` extra.
- Added the `iqm.qrisp_iqm.passes` module, which provides several passes for
  Qrisp's `PassManager` system. Among them a VF2++ layout pass (`vf2pp_layout`),
  a SABRE based routing and layout pass (`plasma_route` , `plasma_layout`).
- Added the `iqm.qrisp.qec` module, which streamlines setting up and
  executing QEC experiments on IQM hardware.
...
- Removed the obsolete old quantum architecture models:
  `QuantumArchitectureSpecification`, `QuantumArchitecture`.
- `.IQMClient.about` now provides the versions of IQM OS, IQM Server and
  Station Control on the QC the client is connected to.
  It no longer provides the versions of all the Python dist packages
  Station Control uses.

### Bug fixes

- `.IQMBackendBase` is now only aware of qubits with the "measure"
  operation available, since this is required by the Qiskit transpiler.
  All other qubits are filtered away from the DQA before it is used.
- Use correct argument `qubit_index_to_name` when calling `.IQMBackend.run` in
    Qiskit user guide.
- Make QASM parser work with cirq-core 1.5
- `IQMJob.result` now correctly retrieves results also for jobs that already
  are in a terminal state.

### Features

- Allow passing quantum computer name to `qiskit_iqm` examples.
- Docs updated.
- The `_IQMServerClient` class is now included in the API docs.
- Added `.IQMClient._debug_info`, which collects useful information to
  include in bug reports and customer support requests.
...
- `.IQMClient` now connects to IQM Server instead of directly to Station Control service.
  - The `url` init parameter for `.IQMClient` is replaced by `iqm_server_url`.
  ...
  - The `timeout_secs` parameter is removed for all `.IQMClient` methods, value from the
    `IQM_CLIENT_REQUESTS_TIMEOUT` environment variable is used instead (default 120 seconds).
  - `IQMClient.get_quality_metric_set` and `IQMClient.get_calibration_set` now return
    slightly different data models with more complete observation data.
  - Circuit and job related models have been moved to `iqm.station_control.interface.models`
    from IQM client models, to avoid code duplication and use the same models in all SW
    components. Imports have to be updated.
  - `IQMClient.submit_circuits` no longer has the parameter `custom_settings`. Use Pulla if
    you need this functionalty.
  - `IQMClient.submit_circuits` and `IQMClient.submit_run_request` now return a
    `.CircuitJob` object instead of the plain job ID.  The job object contains the job ID,
    and provides methods for monitoring the job's state and retrieving its results.
  - `IQMClient.get_run` is replaced by `IQMClient.get_job`, which returns a
    `.CircuitJob` object. The job results can be queried using `CircuitJob.result`.
  - `IQMClient.get_run_status` is removed, you can check the job object for its status using
    `CircuitJob.update`.
  - `IQMClient.wait_for_compilation` and `IQMClient.wait_for_results` are removed,
    use `CircuitJob.wait_for_completion` instead.
  - `IQMClient.get_run_counts` is replaced by `IQMClient.get_job_measurement_counts`.
  - `IQMClient.abort_job` is replaced by `IQMClient.cancel_job`.
- The `iqm.iqm_server_client.authentication` module including `TokenManager` has been moved to
  `iqm.station_control.client.authentication`, and its API has slightly changed.

### Features

- `IQMClient.submit_circuits` and `IQMClient.submit_run_request` have a new parameter,
...
- Identified potential bug with semantic equivalence of transpiled circuits when using the IQMNaiveMovePass. Tests are temporarily marked as expected to fail.

## Version 32.1.0 (2025-10-13)

### Features

- Add support for the `with circuit.if_test()` construction for classical control in Qiskit.
- Deprecate the `c_if` method in favor of the new `if_test` method for classical control in Qiskit.
- Bugfix: Fixed the transpiler bug causing classical conditionals to be ignored in certain cases.
...
- Remove `iqm.qiskit_iqm.examples.resonance_example`.
...
- Use `iqm.pulse.Circuit` and `iqm.pulse.CircuitOperation` as the new
  circuit and instruction formats. The old `Circuit`, `Instruction` and `NativeOperation` models defined in
  `iqm.iqm_client.models` are removed.
- The quantum operation parameters `angle_t` and `phase_t` (in units of full turns) are no longer supported.
  Use `angle` and `phase` (in units of radians) instead.
- `iqm.cirq_iqm.serialize.instruction_to_operation` renamed to `iqm.cirq_iqm.serialize.circuit_operation_to_operation`.
- `iqm.cirq_iqm.serialize.map_operation` renamed to `iqm.cirq_iqm.serialize.operation_to_circuit_operation`.
...
- Add the experimental `IQMClient.get_calibration_quality_metrics` method to retrieve calibration set
  and related quality metrics from the server.
- Select calibration and quality metric data are now available in `IQMTarget`,
  `IQMBackend`, and `IQMBackendBase` for transpilation and querying.
  This data includes the gate durations.
- `transpile_to_IQM` no longer has the `target` parameter, the transpilation target is always
  obtained from `backend`.
- `IQMTarget` moved into its own module.
- `IQMFacadeBackend` can be given the name of the `IQMFakeBackend` instance to use.

### Breaking changes

- `generate_initial_layout` takes an `IQMTarget` as an input argument instead of `IQMBackend`.
...
- Minimal changes needed to support Qiskit version 1.3 and version 1.4.

## Version 29.11.0 (2025-08-07)

### Features

- Added `IQMClient._get_calibration_quality_metrics` for obtaining quality metric and calibration information.
...
- Major `IQMClient` refactoring. Many methods now use `StationControlClient` internally to retrieve the data,
  ...
  is to remove all HTTP logic from `IQMClient` and handle HTTP requests/responses only in `StationControlClient`.

### Breaking changes

- Remove `APIVariant` from `IQMClient`, it will support only one version at any given time from now on.
  Upgrade/downgrade software if you want to use different versions of `IQMClient`.
- You may have to pass a different `url` to `IQMClient` than before:
  - If you previously used `APIVariant.V1`, you typically need to change `/cocos` to `/station` in the URL.
  - If you previously used `APIVariant.V2`, you typically need to add `/station` to the URL.
  - If you previously used `APIVariant.RESONANCE_COCOS_V1`, no changes are needed to the URL.
- Support for `timeout_secs` parameter has been dropped from some methods (`get_static_quantum_architecture`,
  `get_quality_metric_set`, `get_calibration_set`, `get_dynamic_quantum_architecture`, `get_feedback_groups`).
  `IQMClient` uses `StationControlClient` to retrieve the data now instead of HTTP requests directly, and we have no
  plans to support different request timeout for each endpoint separately in `StationControlClient`. Instead, all
...
- Updated `iqm_client.models.Status` to conform with changed station-control-client job statuses.

## Version 27.1.0 (2025-06-09)

### Bug fixes

- Added a backwards compatibility fix for Resonance.
- Made Resonance integration tests mandatory again.
...
- Altered `iqm_client.models.status` to cover changed station-control-client job statuses.
...
- Extended `iqm_client.models.status` to cover new station-control job statuses.
- Updated to work with station-control version 42.0. Earlier versions of station-control will not work.

### Breaking changes

- `iqm_client.models.Status.PENDING_EXECUTION` changed from "pending execution" to "pending_execution".
- `iqm_client.models.status.PENDING_COMPILATION` changed from "pending compilation" to "pending_compilation".

## Version 24.3.0 (2025-05-14)

Feature

- Allow the `.IQMMoveLayout` transpiler to handle gates that are not in the native gateset, done by skipping the marking
  of Circuit Qubits as either qubit or resonator when the gate is not natively supported.
...
- Support the `cirq.R` reset operation in `cirq_iqm`.

## Version 24.0.0 (2025-04-17)

### Features

- Add `timeout` argument and flag to `IQMJob.result`.
...
- Fix missing api docs for `iqm.qiskit_iqm` and `iqm.cirq_iqm`.
...
- `iqm.cortex_cli` is moved inside `iqm-client` to a new submodule `iqm.iqm_client.cli`. The corresponding `cli`
  extra dependency can be installed as `iqm-client[cli]`.
...
- Add `cirq_iqm` to `iqm-client` distribution package as an optional feature.

## Version 22.12.0 (2025-03-31)

### Features

- `IQMClient.get_static_quantum_architecture` added.
- `iqm.qiskit_iqm.fake_backends.IQMFakeBackend` uses the static quantum architecture.
...
- Add `qiskit_iqm` to `iqm-client` distribution package as an optional feature.
...
- `Instruction.args` now has a default value (an empty dict) for convenience.

### Bug fixes

- `transpile_insert_moves` now handles `barrier` instructions properly.
...
- Improve the performance of `.transpile_insert_moves`.
...
- By default `.transpile_insert_moves` now keeps any existing MOVE gates in the circuit.
  `#156 <https://github.com/iqm-finland/iqm-client/pull/156>`_
...
- Add `get_feedback_groups` method. `#162 <https://github.com/iqm-finland/iqm-client/pull/162>`_
...
- Add `get_run_counts` method. `#158 <https://github.com/iqm-finland/iqm-client/pull/158>`_
- Add `get_supported_client_libraries` method. `#158 <https://github.com/iqm-finland/iqm-client/pull/158>`_
...
- Bugfix: Fix the issue where the `CircuitCompilationOptions` was not used in local circuit
  validation when using the `submit_circuit` method. Improved testing to catch the bug.
...
- Add `IQMClient.get_dynamic_quantum_architecture`. `#131 <https://github.com/iqm-finland/iqm-client/pull/131>`_
