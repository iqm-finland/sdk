...
## Version 28.0.2 (2026-08-26)

### Features

- Bump `iqm-data-definitions` dependency to support major version 3.

## Version 28.0.1 (2026-08-05)

### Bug fixes

- Pinned iqm-data-definitions to 2.24.

## Version 28.0.0 (2026-07-07)

### Breaking changes

- `ChipTopology` no longer contains locus mapping information, it's now handled
  by the `GateImplementation` classes themselves.
- `ChipTopology.get_loci` sorts the returned loci for symmetric gates.
- Removes deprecated `copy` method from `BaseModel`.
- `scale` argument of `normalize_coordinates()`has been replaced
  with `min_spacing` (minimum coordinate spacing).

### Bug fixes

- Switch from pinned `requests` dependency to range.
- Domain changed from meetiqm.com to iqm.tech
- Fix Value and Uncertainty "any" schema which fails under
  strict schema validation
- Switch from pinned dependencies to ranges for the bare package installation.

### Features

- Renamed core exception classes to be transport-agnostic. Error names now reflect
  the domain logic and failure nature rather than HTTP-specific status codes.
  The legacy HTTP-based error names are maintained as aliases to prevent breaking
  changes for existing consumers. These are slated for future deprecation
- Refactored exception hierarchy: Replaced `ExaError` with a new
  abstract base class, `IQMError`, to unify error handling across
  all software layers. To ensure backward compatibility, `ExaError`
  remains available as an alias for `ValidationError`.
- Qubits without probe lines are now stored in the new `qubits_without_probeline`
  added to `ChipLayout`.
- Supports angles in `ChipLayout` that can be extracted from CHAD.
- Add optional dependency groups with pinned versions to `iqm-exa-common`:
...
- `ChipTopology` sorts probe lines alphanumerically, not lexically like before.
