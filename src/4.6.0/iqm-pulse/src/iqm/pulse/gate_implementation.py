...
from iqm.pulse.locus_mappings import Locus, LocusMapping, StationProperties, one_qubit
...
class GateImplementation(abc.ABC):
    ...
    _needs_recalibration: bool = True
    """Iff False the implementation has ``parameters`` but they are normally fixed, chosen manually,
    and do not need to be (re)calibrated.

    Used by the calset creation logic.
    """
    # TODO decide where this information belongs. Maybe each one of ``GateImplementation.parameters``
    # should have a flag to denote whether it needs actual recalibration?

    locus_mapping_function: Callable[[StationProperties], LocusMapping] = one_qubit
    """Locus mapping function :meth:`get_locus_mapping` uses.

    For convenience the default is "all one-qubit loci".
    """
    ...
    def needs_calibration(cls) -> bool:
        ...
        """
        # TODO should be called has_parameters, since it may be that the parameter values are
        # somewhat arbitrary and do not need actual calibration. Cf. ``GateImplementation._needs_recalibration``.
    ...
    def convert_calibration_data(
        ...
        * The ``duration`` parameter is converted to channel samples and included in the converted
        ...
        def convert(unit: str, value: Any, dur: float) -> Any:
            """Convert time-like values to units of ``duration`` and frequency-like values to units of its inverse."""
            if value is None:
                return None
            if unit not in {"s", "Hz"}:
                return value
    ...
    def get_parameters(
        ...
        *,
        ...
        Formats the :attr:`parameters` class attribute into a tree of :class:`.SettingNode` objects,
        replacing any wildcard characters.
    ...
    def build_setting_node(
        ...
        *,
        ...
        By default, uses :meth:`get_parameters` to build the node, but can be overridden by subclasses if needed
        (e.g. if the node structure needs to be aware of the ``station_properties``).

        Args:
            locus: Locus component names to replace the wildcard character ``"*"`` in the calibration
                parameter names. One ``Setting`` will be generated for each component name in ``locus``.
                If there are no wildcard characters in ``cls.parameters``, this argument has no effect.
            station_properties: The station properties. By default, not used, but subclasses may override this method
                in order to implement topology or control channel specific node structures.
            path: parts of the dotted name for the root node, if any.
            use_defaults: Iff True, return default values for calibration data items that have one.
        ...
        """
        return cls.get_parameters(locus, path, use_defaults=use_defaults)

    @final
    @classmethod
    def get_locus_mapping(cls, station: StationProperties) -> LocusMapping:
        """Get the locus mapping for this GateImplementation.

        The locus mapping defines all the possible loci where this GateImplementation can be applied.
        It takes into account the symmetry of the gate and the implementation, and should not
        include unnecessary duplicate nodes for symmetric implementations, only the one sorted in the
        canonical (numerical) order.

        For arity-0 gates the locus mapping should only have single-component loci.

        Args:
            station: Static properties of the station for which to create the locus mapping.

        Returns:
            Mapping from the possible loci for this GateImplementation to the QPU components used
                by the implementation on that locus.

        """
        return cls.locus_mapping_function(station)
    ...
    def build(
        ...
        *,
        ...
        def get_custom_oi(cal_impl: GateImplementation) -> OICalibrationData:
            """Find custom calibration for a member op/implementation.

            Returns the custom calibration data node for the requested member op/implementation in
