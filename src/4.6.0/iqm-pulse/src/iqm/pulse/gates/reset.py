...
from iqm.pulse.locus_mappings import one_component_channel, one_qubit_drive_readout
...
class Reset_Conditional(CompositeGate):
    ...
    locus_mapping_function = one_qubit_drive_readout
    ...
    def _call(self) -> TimeBox:  # type: ignore[override]
        ...
        probe_timeboxes = self.build("measure", resettable)(RESET_MEASUREMENT_KEY, feedback_key=RESET_FEEDBACK_KEY)
        ...
        for probe_timebox in probe_timeboxes:  # type: ignore[union-attr]
            probe_timebox.neighborhood_components[0] = blocks
        return TimeBox.composite([probe_timeboxes, resets])  # type: ignore[list-item]
...
class Reset_Wait(GateImplementation):
    ...
    locus_mapping_function = one_component_channel
