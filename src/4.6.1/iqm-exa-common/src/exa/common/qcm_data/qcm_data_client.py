...
class QCMDataClient:
    ...
    def __init__(self, root_url: str, fallback_root_url: str = "") -> None:
        self.logger = logging.getLogger(__name__ + "." + self.__class__.__name__)
        self.logger.info("Initialize QCMDataClient: root_url=%s", root_url)

        if not root_url:
            raise ValidationError("QCMDataClient 'root_url' cannot be empty, it must be a valid HTTP or file URL.")
    ...
    def get_chip_design_record(self, chip_label: str) -> dict:
        ...
        self._validate_chip_design_record(data)
        return data
    ...
    @staticmethod
    def _validate_chip_design_record(chip_design_record: dict[str, Any]) -> None:
        ...
        if (
            ...
            raise ValidationError(f"CHAD content format version '{content_format_version.public}' is not supported.")
