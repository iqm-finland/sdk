...
## Version 2.0.2 (2026-08-26)

### Bug fixes

- The Jupyter notebook `Which Qubits on the QPU Are Used.ipynb` has been removed
  for being outdated.
- All other Jupyter notebooks had their minor fixes to get them updated:
  - The notebooks have fixed IQM server URL (and separated the quantum
    computer name).
  - The `Circuit Synthesis.ipynb` notebook has fixed the name of the method
    called to create the Qiskit circuit (from `build_qiskit` to
    `build_qiskit_phase_separator`).
- `SamplerResonance` now accepts the quantum computer name separately to the
  server URL.
- Transpiled Qiskit circuit for the star topology now has the correct number of
  classical bits.
