...
def bootstrap_errors(
    ...
    Returns:
        * X_array, containing all estimated gate tensors of different bootstrapping repetitions along first axis
        * E_array, containing all estimated POVM tensors of different bootstrapping repetitions along first axis
        * rho_array, containing all estimated initial states of different bootstrapping repetitions along first axis
        * df_g_array, containing gate quality measures of bootstrapping repetitions
        * df_o_array, containing SPAM and other quality measures of bootstrapping repetitions
...
def generate_non_gate_results(
    ...
    Returns:
        df_o_final, the final formatted results
...
def generate_unit_rank_gate_results(
    ...
    Returns:
        * df_g_final, the dataframe with properly formatted results of standard gate errors
        * df_g_rotation, a dataframe containing Hamiltonian (rotation) parameters
        * Dictionary containing the Hamiltonian parameters for each gate in the Pauli basis.
...
def generate_gate_results(
    ...
    Returns:
        * df_g_final, the dataframe with properly formated results of standard gate errors
        * df_g_evals_final, a  dataframe containing eigenvalues of the Choi matrices for all gates
...
def run_mgst_wrapper(
    ...
    Returns:
        * K, Kraus estimate array where each subarray along the first axis contains a set of Kraus operators.
          The second axis enumerates Kraus operators for a gate specified by the first axis.
        * X, superoperator estimate array where reconstructed CPT superoperators in
          standard basis are stacked along the first axis.
        * E, current POVM estimate
        * rho, current initial state estimate
        * K_target, target gate Kraus array where each subarray along the first axis contains a set of Kraus operators.
          The second axis enumerates Kraus operators for a gate specified by the first axis.
        * X_target, target gate superoperator estimate array where reconstructed CPT superoperators in
          standard basis are stacked along the first axis.
...
def process_layout(
    ...
    Returns:
        ...
        * df_g_final, DataFrame containing gate metrics (fidelity, diamond distance, etc.)
        * df_o_final, DataFrame containing non-gate metrics (SPAM errors, fit quality)
        * df_g_evals, DataFrame containing Choi matrix eigenvalues (for rank > 1)
...
def process_plots(
    ...
    Returns:
        ...
        Keys follow the pattern ``f"layout_{qubit_layout}_{plot_type}"``.
...
def mgst_analysis(run: BenchmarkRunResult) -> BenchmarkAnalysisResult:
    ...
    if dataset.attrs["parallelization_path"] == "bootstrap":
        all_results = [process_layout(args) for args in args_list]
    else:
