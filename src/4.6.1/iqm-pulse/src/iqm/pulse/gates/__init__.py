...
from iqm.pulse.gate_implementation import CompositeGate, GateImplementation
...
from iqm.pulse.gates.coupler_move import CouplerMOVE_CRF, CouplerMOVE_CRF_CRF
...
from iqm.pulse.gates.drive_crosstalk import DriveCrossTalk
...
def register_implementation(
    ...
    *,
    ...
    # Check for CompositeGate dependencies
    if issubclass(impl_class, CompositeGate):
        missing_gates = [gate for gate in impl_class.registered_gates if gate not in operations]
        if missing_gates:
            raise ValueError(
                f"Cannot register '{impl_name}' for '{op_name}': "
                f"Missing required constituent gates {missing_gates} in operations."
            )
