...
from exa.common.errors.iqm_error import ConflictError, NotFoundError, ValidationError
...
class CHAD(ImmutableBaseModel):  # noqa: D101
    # TODO most methods of this class should be consolidated to ChipTopology, or removed.
    mask_set_name: str
    variant: str
    ...
    @deprecated(format_deprecated(old="filter_qubit_components", new=None, since="2026-04-14"))
    ...
    def group_components_per_default_operations(
        ...
        components: list[Qubit | Coupler] = [*self.components.qubits, *self.components.couplers]
    ...
    @deprecated(format_deprecated(old="get_probe_line_mapping_for", new=None, since="2026-04-14"))
    ...
    def get_common_coupler_for(self, first_component: str, second_component: str) -> str:
        ...
        Raises:
            - ValidationError: If there were no couplers or more than one coupler connecting
                the component pair (the latter should not be possible in a realistic chip).
    ...
    def _validate_input(self, component_names: Collection[str]) -> None:
        if len(set(component_names)) < len(component_names):
            raise ConflictError(f"The provided component names {component_names} contain duplicates.")
        chad_component_names = self.components.all.keys()
        for component_name in component_names:
            if component_name not in chad_component_names:
                raise NotFoundError(f"The provided component name '{component_name}' doesn't exist in the CHAD.")
