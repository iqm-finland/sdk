...
def generate_rotation_param_results(
    ...
    Returns:
        - Pandas DataFrame containing Hamiltonian (rotation) parameters with formatted values
        - Dictionary with raw values and uncertainty bounds if bootstrapping was used
...
def dephasing_probabilities_2q(X_opt_pp: np.ndarray, X_ideal_pp: np.ndarray) -> list[np.ndarray]:
    ...
    """
    dephasing_probs = [
        minimize(dephasing_dist, [0.1, 0.1], args=X_opt_pp[i] @ la.inv(X_ideal_pp[i])).x  # type: ignore[arg-type]
...
def bootstrap_errors(
    ...
    Returns:
        * X_array, containing all estimated gate tensors of different bootstrapping repretitions along first axis
        * E_array, containing all estimated POVM tensors of different bootstrapping repretitions along first axis
        * rho_array, containing all estimated initial states of different bootstrapping repretitions along
          first axis
...
def save_var_latex(key: str, value: str | float) -> None:
