...
This guide serves as an introduction to the main features of the Cirq adapter of IQM Client.
...
.. note::
    ...
    you should first uninstall it with ``$ uv pip uninstall cirq-iqm``. In this case, you should also include
...
.. code-block:: bash
   ...
   $ uv pip install "iqm-client[cirq]"


After installation, the Cirq adapter can be imported in your Python code as follows:
...
The Cirq adapter provides descriptions of IQM's quantum architectures using the :class:`.IQMDevice` class, which is a
...
benefit from the Cirq adapter's :mod:`.optimizers` module which contains some optimization tools
geared towards IQM devices.
...
.. note::
   ...
   You can access IQM quantum computers via `IQM Resonance <https://iqm.tech/products/iqm-resonance>`_
...
The Cirq adapter provides :class:`.IQMSampler`, a subclass of :class:`cirq.work.Sampler`, which is used
...
.. code-block:: python
   ...
   sampler = IQMSampler(iqm_server_url, token=token, quantum_computer=quantum_computer)
   ...
   print(result.measurements['q1'])


Note that the code snippet above assumes that you have set the variable ``iqm_server_url`` to the URL
of the IQM Server, ``token`` for the API token obtained from the web dashboard,
and ``quantum_computer`` to the name of the quantum computer to use, if the IQM Server
...
.. code-block:: python
   ...
   sampler = IQMSampler(iqm_server_url, token=token, calibration_set_id="f7d9642e-b0ca-4f2d-af2a-30195bd7a76d")
...
.. code-block:: python
   ...
   circuit_list = [routed_circuit_1, routed_circuit_2]
...
`Cirq examples directory <https://github.com/iqm-finland/sdk/tree/main/iqm_client/src/iqm/cirq_iqm/examples>`_
