...
from exa.common.errors.iqm_error import InvalidOperationError
...
def convert_to_options(config: dict[str, Any]) -> SweepOptions:
    ...
    Raises:
        BadRequestError: Error is raised if config has unsupported structure
    ...
    raise InvalidOperationError(f"Config {config} cannot be converted to range options")
...
def __rename_key(config: dict[str, Any], old_key: str | list[str], new_key: str | list[str]) -> None:
    if not (isinstance(old_key, str) and isinstance(new_key, str)):
        raise InvalidOperationError(f"Types {type(old_key)=} and {type(new_key)} must be str.")
