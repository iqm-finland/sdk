...
value is added as child to the final node. Note: ``SettingNode`` always aligns the path and name of any nodes under it,
...
from exa.common.errors.iqm_error import InvalidOperationError, NotFoundError, UnknownSettingError, ValidationError
...
class SettingNode(BaseModel):  # noqa: PLW1641 (Object does not implement `__hash__` method)
    ...
    Args:
        ...
        settings: Dict of setting path fragment names (usually the same as the setting name) to the settings. Mostly
            used when deserialising and otherwise left empty.
        subtrees: Dict of child node path fragment names (usually the same as the child node name) to the settings.
        ...
        **kwargs: Any,
        ...
        for key, child in settings.items():
            if isinstance(child, Setting):
                ...
                raise InvalidOperationError(f"Unsupported type for settings['{key}']: {type(child)}")
        ...
        for key, child in subtrees.items():
            if isinstance(child, SettingNode):
                ...
                raise InvalidOperationError(f"Unsupported type for subtrees['{key}']: {type(child)}")
    ...
    def __setitem__(self, key: str, value: Any) -> None:
        ...
        if not remaining_path:
            ...
            try:
                ...
                raise ValidationError(f"Path '{key}' is invalid: '{node_key}' is a setting, not a node.")
        else:
    ...
    def print_tree(self, levels: int = 5) -> None:
        ...
        def append_lines(node: SettingNode, lines: list[str], indents: list[bool]):  # noqa: ANN202
            ...
            if len(indents) < levels:
                ...
                for subtrees_written, (key, subtree) in enumerate(node.subtrees.items()):
    ...
    def diff(self, other: SettingNode, *, path: str = "") -> list[str]:
        ...
        node_diff.extend([f"+setting: {key}" for key in b_keys if key not in self.settings])
        ...
        node_diff.extend([f"+subnode: {key}" for key in b_keys if key not in self.subtrees])
    ...
    def get_node_for_path(self, path: str) -> Setting | SettingNode:
        ...
        Raises:
            ValidationError: If the given path is invalid.
            NotFoundError: If the given path cannot be found in self.
        ...
        for index, key in enumerate(keys, start=1):
            if index < len(keys) and key in node.settings:
                raise ValidationError(f"Path '{path}' is invalid: '{key}' is a setting, not a node.")
            if key not in node.children:
                raise NotFoundError(f"Path '{path}' is invalid: key '{key}' is not found in the preceding node {node}.")
            node = node[key]
        return node
    ...
    def get_default_implementation_name(self, gate: str, locus: str | Iterable[str]) -> str:
        ...
        Args:
            ...
            locus: Individual qubits, couplers, or combinations. Locus or locus string.
        ...
        gate_def = self.gate_definitions[gate]
        for impl_name, impl_def in gate_def.child_nodes:
            if impl_def.override_default_for_loci.value:
                # for symmetric gates, try all locus permutations to see if one has an override
                loci = permutations(locus) if gate_def.symmetric.value else [tuple(locus)]
                for permuted_locus in loci:
                    locus_str = "__".join(permuted_locus)
                    if locus_str in impl_def.override_default_for_loci.value:
                        return impl_name
        return gate_def.default_implementation.value

    def _get_possible_loci(self, gate: str, implementation: str, locus: tuple[str, ...]) -> list[str]:
        """Return the possible EXA settings tree locus strings for the given (gate, implementation, locus) triple.

        This convenience method allows the EXA user to input loci in different orders for symmetric gates.
        """
        if len(locus) == 1:
            return list(locus)  # shortcut
        if self.gate_definitions[gate][implementation].symmetric.value:
            # symmetric implementation means the gate has to be symmetric too, and the locus is always sorted
            loci = [sort_components(locus)]
        elif self.gate_definitions[gate].symmetric.value:
            # gate is symmetric, try all permutations to see which one(s) have calibration nodes
            loci = list(permutations(locus))
        else:
            # use the locus as is
            loci = [locus]
        return ["__".join(locus) for locus in loci]
    ...
    def _get_gil_node(
        ...
        """Return the node for the given (gate, implementation, locus) triple under ``node``.

        NOTE: using this method requires the standard EXA settings tree structure.

        Raises:
            NotFoundError: The requested node cannot be found.

        """
        if gate not in node.subtrees:
            raise NotFoundError(f"Gate {gate} cannot be found in the {msg} node.")

        if implementation not in node[gate].subtrees:
            raise NotFoundError(f"Gate {gate}.{implementation} cannot be found in the {msg} node.")

        # return the first applicable locus node
        for locus_str in self._get_possible_loci(gate, implementation, locus):
            if (gate_node := node[gate][implementation].subtrees.get(locus_str)) is not None:
                return gate_node

        raise NotFoundError(f"Locus {locus} for {gate}.{implementation} cannot be found in the {msg} node.")
    ...
    def get_gate_node_for_locus(
        ...
        """Get the ``gate`` calibration sub-node for ``locus`` if it exists in the settings tree.

        NOTE: using this method requires the standard EXA settings tree structure.

        Args:
            gate: The gate to retrieve the settings for.
            locus: QPU components to apply the gate on. Locus or locus string.
            implementation: Gate implementation to retrieve.
                ``None`` means use the default gate implementation.

        Returns:
            The settings of the specified locus and gate.

        """
        locus = tuple(locus.split("__")) if isinstance(locus, str) else tuple(locus)
        implementation = implementation or self.get_default_implementation_name(gate, locus)
        return self._get_gil_node(self.gates, gate, locus, implementation, msg="gate parameters")

    def get_gate_properties_for_locus(
        self, gate: str, locus: str | Iterable[str], implementation: str | None = None
    ) -> SettingNode:
        """Get the ``gate`` characterization sub-node for ``locus`` if it exists in the settings tree.
        ...
        locus = tuple(locus.split("__")) if isinstance(locus, str) else tuple(locus)
        implementation = implementation or self.get_default_implementation_name(gate, locus)
        return self._get_gil_node(
            self.characterization.gate_properties, gate, locus, implementation, msg="gate properties"
        )
    ...
    def set_shots(self, shots: int, averaging_bins: int | None = None) -> None:
        ...
        if "options" not in controller_settings.subtrees:
            raise NotFoundError("CommonOptionsControllers cannot be found in the controller settings.")
    ...
    def _rebuild_index(self) -> None:
        ...
        self._index_by_name = dict(index.items())
