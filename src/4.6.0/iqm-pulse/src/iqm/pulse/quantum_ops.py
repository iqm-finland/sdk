...
class QuantumOp:
    ...
    * locus defines the target of the operation (where)
    ...
    Zero arity implies either that the operation is :attr:`factorizable` (e.g. ``measure``),
    or that its implementations do not require any calibration data (e.g. ``barrier``).
    """
    ...
    aggregator_class: type[GateImplementation] | None = None
    """Class that implements the aggregation of ``arity==1`` factorizable sub-implementations
    into the full ``arity > 1`` implementation. Iff None, the QuantumOp is not factorizable.
    See :meth:`factorizable`."""
    ...
    def factorizable(self) -> bool:
        """Whether the QuantumOp is factorizable.

        A factorizable QuantumOp is implemented by aggregating independent single-subsystem operations,
        and the calibration for an ``arity > 1`` locus is derived from the ``arity==1` locus calibrations
        (for example the ``measure`` operation). See :attr:`aggregator_class`.
        """
        return self.aggregator_class is not None
