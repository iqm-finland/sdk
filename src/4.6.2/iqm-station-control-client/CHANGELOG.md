...
## Version 13.0.2 (2026-08-26)

### Features

- Bump `iqm-data-definitions` dependency to support major version 3.

## Version 13.0.1 (2026-08-05)

### Bug fixes

- Pinned iqm-data-definitions to 2.24.

## Version 13.0.0 (2026-07-07)

### Breaking changes

- `QONControllerSetting` attribute names changed. Now reflects better the observations.

### Bug fixes

- Prevented Core API artifact download premature EOS for the streaming job artifacts
...
- Added absolute timeout logic to the `StationControlClient.run()` method to
  prevent infinite polling. Defaults to 3 hours (10800 seconds), but can be
  configured using the `StationControlClient.timeout_secs` attribute.
- Improved security in StationControlClient by stripping authorization
...
- Add optional dependency groups with pinned versions to `iqm-station-control-
  client`:
...
- Added SweepDefinitionLite as a lightweight alternative to SweepDefinition.
