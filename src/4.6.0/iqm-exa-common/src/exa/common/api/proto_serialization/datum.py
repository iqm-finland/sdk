...
def pack(value: None | bool | str | complex | np.ndarray | Sequence) -> dpb_Datum:
    ...
    Raises:
        ValidationError in case of unsupported type.
    ...
    if value is None:
        return dpb_Datum(null_value=1)  # type: ignore[arg-type]
    ...
    raise ValidationError(f"Encoding of type '{type(value)}' is not supported.")
...
def unpack(source: dpb_Datum) -> None | str | bool | int | float | complex | np.ndarray | list:
    ...
    Raises:
        ValidationError or google.protobuf.message.DecodeError in case of invalid buffer
    ...
    raise ValidationError(f"Unrecognized datatype field {field_name}")


def serialize(value: None | bool | str | complex | np.ndarray | Sequence) -> bytes:
