...
## Version 14.0.2 (2026-08-26)

### Features

- Bump `iqm-data-definitions` dependency to support major version 3.

## Version 14.0.1 (2026-08-05)

### Bug fixes

- Pinned iqm-data-definitions to 2.24.
- block_neighbors = True logic fixed (could generate more than 2
  ReadoutTriggers).
- measure multiplexing function now works correctly with more
  than 2 groups (e.g. created by the aforementioned neighbor blocking).

## Version 14.0.0 (2026-07-07)

### Breaking changes

- ``QuantumOp`` now has an attribute ``aggregator_class`` that is populated
  for factorizable operations, instead of ``QuantumOp.factorizable`` (the
  latter is retained as a property that is derived from the former).
- `GateImplementation.special_implementation` removed as unnecessary.
- `ScheduleBuilder.__init__`  signature changed to use `StationProperties`.
- `StationProperties` contents changed.
- Removed `iqm.pulse.playlist.channel.get_channel_properties`.
- Removed `iqm.pulse.playlist.get_channel_properties_from_station_settings`.

- `GateImplementation.get_locus_mapping_name` and `GateImplementation.get_custom_locus_mapping`
  are removed. Instead the locus mapping is now always created by
  `GateImplementation.get_locus_mapping`. Locus mapping names do not exist anymore.

### Bug fixes

- CompositeGates that do not need calibration can no longer be instantiated for
  arbitrary loci.
...
- Added class MeasureBoxes that allows uniform handling (including
  multiplexing) of measure implementation return types.
- All measure implementations now return MeasureBoxes with their
  call method.
- Measure implementations are refactored, reducing boilerplate and
  improving extendability.
- ProbePulse_CustomWaveForms_noIntegration renamed to
  NoAcqusition_Measure_CustomWaveForms for consistency.
- Add a subclass of `TripleFluxPulseGate` using the Slepian pulse shape
  for the coupler.
- Register and expose both CRF and Slepian triple fast flux implementations.
- Add new gate implementation class `TripleFluxPulseGate` for two-qubit
  gates with fast flux on both qubits and the coupler.
- Add `Move_Measure_Composite` implementation that allows measuring
  components without probe lines (such as computational resonators).
- Add `has_virtual_drive` attribute to `StationProperties`
  to be able to distinguish components that have a physical drive
  controller from components with a virtual drive channel,
  which can only apply virtual phase corrections but no PRX gates.
- Adds a new gate `coupler_move`
- Adds new implementations for this gate `CouplerMOVE_CRF` and `CouplerMOVE_CRF_CRF`
- Move_Measure_Composite implementation directly sets readout label for
  the correct component. We don't need to fix the label in the
  post-processing anymore.
- Expose `modulation_frequency` parameter of `IQPulse`
  in `PRX_CustomWaveforms` and `PRX_CustomWaveformsSX`
  classes to allow for the calibration of off-resonant DRAG
  pulses. The change is backwards compatible, and
  uses a default value of `0.0` for `modulation_frequency`.
- GateImplementations can now build station properties aware setting nodes
  via the method `build_setting_node`.
- Add a function to calculate the parameters of the U gate based on desired unitary.
- Add optional dependency groups with pinned versions to `iqm-pulse`:
...
- Add the experimental `drive_crosstalk` gate implementation
  `DriveCrossTalk`, a global-locus calibration container holding the drive
  crosstalk matrix in sparse form (`matrix_index`,
  `crosstalk_amplitudes`, `crosstalk_phases`).
  Calling it with a resolved schedule and a list of `<target>__<control>`
  loci returns a `TimeBox` with the drive crosstalk compensation pulses
  (the control's drive replayed on the target's `.aux0` channel, IQ-scaled
  by the crosstalk amplitude and phase-shifted via the `phase_increment` of
  the first replayed `IQPulse`), consumed by the `apply_drive_crosstalk`
  compiler pass.
  Registered as a default gate.
- Add new measure implementation `QBBias_Measure_Constant` with fast flux bias
  during readout.
- Only register a composite gate, if all required constituent gates
  are registered, otherwise throw an error.
- `iqm.pulse.locus_mappings.two_component_one_flux_coupler_flux` returns the
  loci in sorted order, for nicer presentation in the EXA settings tree.

## Version 13.0.0

### Breaking changes

- The implementation priority order in `.QuantumOp` is gone, replaced
  by `.QuantumOp.default_implementation` and `.QuantumOp.defaults_for_locus`.
- `.ScheduleBuilder.get_implementation` now uses the parameter
  `priority_calibration_factorizable` to override calibration for factorizable gates.

### Bug fixes

- Standalone `VirtualRZ` instruction durations now conform to the minimum
  allowed duration across all control channels.

### Features

- `PRX_CustomWaveForms` supports sampled (non-parametric) Waveforms.
- Added `PRX_Samples` that allows easily providing sampled waveforms as
  calibration data.
- In class `RZ_PRX_Composite`, change the decomposition of the RZ gate
  into PRX gates to enable calibrating the MOVE operation with the SX
  gate implementation.
- Add more examples to the documentation regarding `.ScheduleBuilder` usage.
- Commonly used classes now available in the top level module `iqm.pulse`.
- Add a virtual drive channel not only for computational resonators, but also
  for qubits in case they do not have a drive controller.
- Add new quantum operation, ``lru`` (leakage reduction unit),
  which removes population from the `|2\rangle` state.
- Add implementation ``lru_f0g1`` for the LRU operation,
  which works by driving the F0G1 transition.
- Add a composite gate implementation for the ``reset`` gate
  which works by (repeatedly) applying a ``prx_12(pi)`` gate
  followed by the F0G1 pulse.
- Add a ``reset`` gate implementation
  which uses multiplexed ``prx_12`` and F0G1 pulses
...
- `.QuantumOp.params` is now a dictionary mapping parameter names to their allowed types instead of a tuple of
  parameter names.
- `.CircuitOperation.validate` is more strict about allowed values. For example, the types of
  `.CircuitOperation.args` are now checked.

### Features

- Add `.Circuit` and `.QuantumOp.optional_params`.
...
- Add a new "prx" implementation `PRX_Cosine`, using a cosine pulse for both I and Q components.
- Add a new waveform `PolynomialCosine`, which is a polynomial function of a cosine,
  `f(n) = \sum_{k=0}^m a_k \cos^k(2 \pi f n)`, where `n` is the sample index.

### Bug fixes

- In the waveform `Cosine` add `phase` to `non_timelike_attributes`.

## Version 10.0.0 (2025-07-16)

### Breaking changes

- `.CompositeGate` subclasses must now include all their member gates in `.CompositeGate.registered_gates`.
  The subclasses should apply the member gates using `.CompositeGate.build`.
- Removed the `.PRX_SinglePulse_GateImplementation.iq_pulse` alias, use `.pulse` instead.
- `.register_implementation` no longer can register or define a QuantumOp.
  `.register_operation` is introduced for that purpose.
...
- Add new MOVE implementation `MOVE_SLEPIAN_CRF`.
...
- Convert `numpy` types to Python's built-in types so that playlist inspector HTML is rendered correctly.
...
- Add a base class for shelved readout `Shelved_Measure_CustomWaveforms` and an implementation of shelved readout
  `Shelved_Measure_Constant`. This implementations consist of `prx_12` + `measure` + `prx_12` gates.
...
- Change deprecated `exa.common.data.parameter.DataType.NUMBER` usage from to `FLOAT` or `INT`.

## Version 9.6.0 (2025-04-28)

### Features

- Added a new probe waveform `ProbePulse_CustomWaveforms_noIntegration` which doesn't integrate

## Version 9.5.0 (2025-04-25)

### Features

- Add the CompositeGate `RZ_PRX_Composite`, which is a physical Z rotation gate implemented as a sequence of
...
- Add Baseclass `PRX_ModulatedCustomWaveForms` for arbitrary IQ waveform modulated PRX gate.
- Add gate implementation `PRX_ModulatedDRAGCosineRiseFall` for cosine rise fall modulated PRX gate.
...
- `.CouplerFluxPulseQubitACStarkPulseGate` also now supports off-locus RZ corrections.
...
- Adds the delay operation `~iqm.pulse.gates.delay.Delay`.

## Version 7.21.0 (2025-02-07)

### Features

- Adds a new special case to `PRX_CustomWaveformsSX` for a PRX rotation angle of zero (i.e., identity), in
...
- Speed up the calculation of unitaries in `.CircuitOperationList` by caching the reshaping function.
- The method `map_loci` gets an optional argument `make_circuit`, by default True, which if False will cause the
  output to be a list of `.CircuitOperation` instead of `.CircuitOperationList`. This is faster and often
  the circuit with mapped locus is immediately appended to another circuit or converted to `.TimeBox`, neither
  of which requires the class structure.
- The method `.ScheduleBuilder.circuit_to_timebox` has an optional argument `locus_mapping`, defaulting to an
  empty dict. If any of the components in the locus of any `.CircuitOperation` is a key in that dict, it is
  replaced with the value at that key. This speeds up scheduling of identical circuits which only differ by locus.

### Bug fixes

- The `__add__`, `__mul__`, and `__getitem__` of `CircuitOperationList` correctly create the
  new object by assinging the `qubits` and `table` attributes at init, and not after, leading to significant
  speedup.
...
- Add two implementations of the classically conditioned prx gate, both subclasses of the existing `.CCPRX_Composite`,
  which fix the `prx` implementation. `.CCPRX_Composite_DRAGCosineRiseFall` fixes it to `drag_crf` and
  `.CCPRX_Composite_DRAGGaussian` fixes it to `drag_gaussian`.
...
- The calibration validation only compares the calibration data relating to the gate itself, and not any of its
  ...
  `.CompositeGate` s,  which both have their own calibration and registered gates to pass validation.
...
- For unitary operations, `QuantumOp.unitary` is a function which takes the operation's params and
  returns the unitary matrix the operation should implement.
- Add a convenience `CircuitOperationList` which is an extension of a builtin list, containing
  ...
  of QuantumOps. It can map the locus to some other locus, and be directly used by the `ScheduleBuilder`
  to create a schedule.
- Add a function `validate_quantum_circuit` extracting the method `ScheduleBuilder.validate_quantum_circuit`
  so it can be used without the full builder, just using a `QuantumOpTable`.
- Add CZ implementations `crf_acstarkcrf` and `slepian_acstarkcrf` to the default `QuantumOpTable`.
- Bump exa-common.
...
- Added a user guide covering basic concepts and Timebox usage.
...
- Fix that using the measure gate on a system without drive lines didn't work.

## Version 7.7.0 (2024-12-30)

### Features

- Bump Station Control Client dependency.
...
- The function `.apply_move_gate_phase_corrections` no longer uses a calibration set, because the value of the detuning
  needed for phase tracking of MOVE sandwiches is now a part of the MOVE gate calibration data proper.

### Features

- Add a parameter `detuning` to parent class of all MOVE implementations, `.MOVE_CustomWaveforms`. This
  parameter only affects the frame tracking, and must be set to the difference of the qubit and resonator frequencies.
- Add this parameter to the `.MoveMarker` instruction.
...
- Add `CouplerFluxPulseQubitACStarkPulseGate` Pulse, which is a base class for AC Stark pulsed CZ gates.

## Version 6.13.0 (2024-11-20)

### Features

- `measure` and `reset_wait` operations now use explicit `.Block` instructions instead of
  `.Wait` s inserted by the scheduling algorithm to idle the qubits, since the former is more
...
- New quantum operation `reset_wait` and its implementation `.Reset_Wait` which is used for resetting qubits
...
- Prefer system fonts in schedule viewer.
...
- Fix scheduling neighborhoods in `.MultiplexedProbeTimeBox.__add__` (affected only Pulla scheduling)
...
- Add implementation `.MOVE_CRF_CRF` for implementing a move operation using cosine rise fall waveform for coupler
  and qubit.
...
- Some cleanup of fast-feedback internals in e.g. `.ScheduleBuilder`, including a fix for the scheduling of
  the edge-case of many qubits listening to a single feedback bit in parallel.

## Version 5.7 (2024-10-16)

- Add a general quantum operation for reset and a gate implementation `.ConditionalReset`
...
- The virtual channels used in fast feedback scheduling no longer block the entire component, allowing more optimal
  schedules.
- The conditional gate `.CCPRX_Composite` now schedules correctly when listening to fast feedback from another
...
- Add docs for the `.phase_transformation` function.
...
- A new gate implementation base class `.SinglePulseGate` added, and `.PRX_CustomWaveforms` now inherits
  from this class, which means the `_single_iq_pulse` method is renamed to `_get_pulse` (this must be done in all
  `.PRX_CustomWaveforms` classes). Otherwise the functionality of the method is the same.
- `.GateImplementation.construct` removed, and the `.GateImplementation.__init__` now fulfills the
  same purpose `construct` had before (all inits must have the same signature).

### Features

- `.GateImplementation.__call__` now handles `TimeBox` caching and the users do not have to
  implement it in every gate implementation separately. Instead you can now just override
  `.GateImplementation._call` if you are satisfied with caching based on the call arguments.

## Version 4.0 (2024-10-02)

### Breaking changes

- `.ConditionalPRX` renamed to `.CCPRX_Composite` which now requires calibration for signal delays.
...
- `.Measure_Constant` now accepts an empty array for the integration weights, signifying constant weigths.
...
- Canonical waveforms are no longer defined via inheritable class property, but instead by a static whitelist.
  Now you can inherit from a canonical waveform without issues.
...
- `Slepian` waveform now supports squid asymmetry to be taken account of.

## Version 3.2 (2024-09-11)

### Features

- Added `GateImplementation.get_custom_locus_mapping` which allows the gate implementations to define their
  own locus mappings, making it possible to write the entire logic of an implementation inside its class definition,
- `GateImplementation` now has a `bool` attribute `special_implementation`, which can be set as `True`
  ...
  - Special implementations cannot be set as default implementations in `QuantumOp`.
...
- New gate implementations in `PRX` using `\pi/2` pulse: `PRX_DRAGGaussianSX` and
  `PRX_DRAGCosineRiseFallSX`, which inherits from `PRX_CustomWaveformsSX`. The default implementation
  names are `drag_crf_sx` and `drag_gaussian_sx`.
- New U gate `UGate` using composition `Z(\phi)Y(\theta)Z(\lambda)`, with `PRX` gate
  `Y(\theta)`. This gate will use the same implementation as `PRX` gate. The default gate name is `u`, and
  implementation name `prx_u`
- New sqrt(X) gate `SXGate` using fixed pulse `X(\pi/2)`, from `PRX` gate. The default gate name is `sx`,
  and implementation name `prx_sx`.

### Breaking changes

- PRX gate attribute `x_pi` and classmethod `_x_pi_pulse` renamed to `iq_pulse` and `_single_iq_pulse`.
...
- Make CompositeGate calibration logic more consistent.

## Version 2.12 (2024-08-27)

### Features

- Add the waveforms `.Chirp` and `.ChirpImag`.
...
- The `ScheduleBuilder`'s priority calibration feature is supported now also for factorizable `QuantumOp`s
...
- `QuantumOp.get_default_implementation_for_locus` returns the locus-specific default for a permutation
...
- Added `QuantumOp.defaults_for_locus` which can be used to set per-locus default implementations of a quantum
  operation.
- If assigned, `ScheduleBuilder.get_implementation` prioritises the locus-specific defaults over any globally
  defined priorities,
...
- `.ScheduleBuilder.validate_quantum_circuit` now accepts mid-circuit measurements

### Bug fixes

- `.ProbePulse_CustomWaveforms` call produces valid integration result labels that have `"__"` in them.
...
- Add composite GateImplementation `.CCPRX_Composite`, usable with programmable readout.

## Version 2.0 (2024-07-15)

### Features

- Add `.TimeBox.print` as a crude way of visualizing TimeBox contents.
- Add `.ScheduleBuilder.resolve_timebox`.
