...
from iqm.pulse.gates.reset import RESET_MEASUREMENT_KEY
...
def _unmap_additional_run_properties(run_data: RunData, mapped_readout_keys: dict[str, str]) -> None:
    ...
    for field in applicable_fields:
        if (arp := additional_run_properties.get(field)) is not None:
            additional_run_properties[field] = (
                {_unmap_label(label, mapped_readout_keys): value for label, value in arp.items()}
                if isinstance(arp, dict)
                else [_unmap_label(label, mapped_readout_keys) for label in arp]
            )
...
def construct_run_result(pulla_data: PullaData, context: dict[str, Any]) -> RunResult:
    ...
    if mapped_readout_keys := context.get("mapped_readout_keys", {}):
        run_data, sweep_results = _unmap_readout_keys(run_data, sweep_results, mapped_readout_keys)
    return _construct_run_result(run_data, sweep_results)


def process_metadata(run: RunResult) -> RunResult:
    """Process the metadata in the RunResult.

    Processes tuples serialised into lists back into tuples.
    ...
    run_components = additional_run_properties["components"]
    grouping_mode = additional_run_properties["components_grouping_mode"]
    if grouping_mode in [ComponentGroupingMode.GROUP.value, ComponentGroupingMode.COLOUR_GROUP.value]:
        if grouping_mode == ComponentGroupingMode.GROUP.value:
            additional_run_properties["components"] = [tuple(group) for group in run_components]
        else:
            colour_groups = [[tuple(group) for group in colour] for colour in run_components]
            additional_run_properties["components"] = colour_groups
    return run
...
def create_ragged_data_index(
    ...
    """
    additional_run_properties = run.run_data.additional_run_properties
    if "ragged_data_labels" not in additional_run_properties:
        return run

    ragged_label_counts = additional_run_properties["ragged_data_labels"]
    hard_sweeps = dict(
        zip(
            additional_run_properties["ragged_hard_sweeps"],
            additional_run_properties["ragged_hard_sweeps_data"],
        )
    )
    parallel_sweep_info = additional_run_properties["ragged_parallel_sweeps"]

    def _generate_parallel_spot(param: str) -> Generator[Any]:
        parallel_datas = [data for p, data in reversed(hard_sweeps.items()) if p in parallel_sweep_info[param]]
        yield from zip(*parallel_datas)
    ...
    def _gen_ragged_index(run: RunResult) -> RunResult:
        ...
        for label, counts in ragged_label_counts.items():
            ...
            for tup_idx, tup in enumerate(raw_tuples):
                ...
                tuples.extend(
                    [new_tup + (found_triggers + trig_idx_in_seg,) for trig_idx_in_seg in range(counts[tup_idx])]
                )
            ...
            ragged_dim_name = next(d for d in ragged_array.dims if RAGGED_DATA_INDEX.name in d)
...
def _rename(
    run_result: RunResult, rename_dict: dict[str, Parameter], data_types_dict: dict[str, DataType]
) -> RunResult:
...
def rename_data_variables(run: RunResult, context: dict[str, Any]) -> RunResult:
    ...
    """
    additional_run_properties = run.run_data.additional_run_properties
    result_labels = additional_run_properties["integration_data_parameters"]
    ...
    return _rename(run, rename_dict, data_types_dict)
...
def _add_contrast(prefix: str, run_result: RunResult) -> dict[str, xr.DataArray] | None:
    ...
    if not readout.dims or len(readout[readout.dims[0]]) < 2:  # noqa: PLR2004
        return None
...
def _get_pca_analyzer(poq_param_name: str) -> Callable[[np.ndarray, np.ndarray], FitResults]:
    def pca(x: np.ndarray, y: np.ndarray) -> tuple[dict[str, float | np.ndarray], np.ndarray]:
...
def compute_excitation_probability_from_data(run: RunResult) -> RunResult:
    ...
    if run.run_data.sweep_data.settings.controllers.counter.subscribed_channels.value:
        ...
        data_prefixes = _get_qubit_and_prefix(
            ...
            additional_run_properties["readout_groups_without_label"],
        )
    else:
        data_prefixes = [_map_default_label(label) for label in result_labels]
    ...
    def _compute_excitation_probability(run_result: RunResult) -> RunResult:
        ...
        new_arrays = {}
        for prefix in data_prefixes:
            component = "COMP_R" if prefix[:6] == "COMP_R" else prefix.split("_")[0]  # fixme
            ...
            if parameter is not None:
                new_arrays[parameter.name] = parameter
        return replace(run_result, dataset=run_result.dataset.merge(new_arrays))
...
def _reconstruct_counter_subscribed_components(additional_run_properties: dict[str, Any]) -> dict[str, list[str]]:
    ...
    readout_groups = additional_run_properties["readout_groups"]
    readout_group_names = additional_run_properties["readout_group_names"]
...
def add_counts_for(run: RunResult) -> RunResult:
    """Add single-qubit excited state probabilities from the multi qubit counter results to the dataset.
...
MeasurementResults: TypeAlias = dict[str, list[list[int]]]
"""Measurement results for a single circuit.

Maps measurement keys to corresponding results.
``results[key][shot][label_index]`` is the measurement result corresponding to
the acquisition label ``GeneralReadoutMapping[key][label_index]``, for the shot ``shot``.
"""

MeasurementResultsBatch: TypeAlias = list[MeasurementResults]
"""Type that represents measurement results for a batch of circuits."""

GeneralReadoutMapping: TypeAlias = dict[str, tuple[str, ...]]
"""Type for matching measurement keys from a quantum circuit with acquisition labels in Station Control.

Maps a readout key to the acquisition labels with that key in the circuit. The order of the labels for
a specific key determines the order of the corresponding results in :attr:`MeasurementResults`.
Each acquisition label corresponds to the readout of a single qubit at a single point in the circuit.
The same acquisition label can be used multiple times.

Note that in general pulse-level execution we don't guarantee a specific order for the labels of a key,
as it can be affected by custom multiplexing and compiler passes.
"""

GeneralReadoutMappingBatch: TypeAlias = tuple[GeneralReadoutMapping, ...]
...
class CircuitExecutionResults:
    ...
    circuit_measurement_results: MeasurementResultsBatch
    """Circuit measurement results."""
    readout_mappings: GeneralReadoutMappingBatch
    """Readout mappings."""
    sweep_results: SweepResults
    """Raw sweep results."""
...
def _process_readout_metrics(
    metrics: ReadoutMetrics, mapped_readout_keys: dict[str, str]
) -> GeneralReadoutMappingBatch:
    """Process a :class:`ReadoutMetrics` into readout mappings."""
    ...
    for seg_idx in range(metrics.num_segments):
        ...
        for readout_label, occurrences in metrics.integration_occurrences.items():
            ...
            _, readout_key = unmapped_label.split("__", maxsplit=1)

            n_occurrences = occurrences[seg_idx]
            if readout_key == RESET_MEASUREMENT_KEY:
                # special case: allow multiple resets on the same qubit in the same segment
                # TODO make the readout postprocessing system more coherent
                mapping[readout_key].extend([unmapped_label] * n_occurrences)
                ...
                raise ValueError(
                    ...
                    f"measure calls with the same readout label in a single segment: '{unmapped_label}'"
                )
