...
from iqm.pulse.utils import get_other_colour
...
def multiplex(sequence_of_boxes: Sequence[MeasureBoxes]) -> MeasureBoxes:
    ...
    final_measure_boxes: list[MeasureBoxes] = list(multiplexed_sequence[0])
    ...
    for next_one in multiplexed_sequence[1:]:
        ...
        # the last Aux box of M{i} is combined with the first Aux box of M{i+1} via MeasureBox.multiplex
        final_measure_boxes[-1] = final_measure_boxes[-1].multiplex(next_one[0])
        final_measure_boxes.extend(list(next_one[1:]))
    ...
    def _call(
        ...
        **kwargs,
        ...
        if self.calibration_data["block_neighbors"]:
            blocked_colour = get_other_colour(self.builder._station_properties, self.locus[0])
        else:
            blocked_colour = set()
        return MeasureBoxes(
            [MeasureBox.composite([]), final_box, MeasureBox.composite([]), MeasureBox.composite([])],
            measure_components=set(self.locus),
            additional_components=blocked_colour,  # block neighboring qubits in multiplexing
        )
