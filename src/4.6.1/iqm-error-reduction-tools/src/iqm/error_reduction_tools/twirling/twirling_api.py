...
class CircuitTwirler:
    ...
    @property
    def readout_twirling_applied(self) -> bool:
        """Whether readout twirling was actually applied to the counts.

        ``True`` only if a non-``NONE`` strategy was requested *and* twirling was
        not skipped by a fallback (missing LOCAL topology, MOVE-gate QPU). This is
        the correct value to pass as ``twirled`` to
        :meth:`~iqm.error_reduction_tools.rem.mitigation.ReadoutErrorMitigation.mitigate_counts`:
        symmetrizing the confusion matrix is only appropriate when the counts were
        readout-twirled.
        ...
        """
        if self._twirl_strategy is None:
            raise RuntimeError("Twirling has not run yet. Call twirl() first.")
        return self._twirl_strategy != "NONE" and not self._twirling_skipped
