...
def _get_gate_and_impl_name(prefix: str, label_to_impl: dict[str, str | None] | None) -> tuple[str | None, str | None]:
    ...
    """
    if len(prefix.split("_")) == 1 or prefix == "COMP_R":  # The default label case
...
def convert_single_shot_to_counter(
    ...
    """Convert multiple arrays of single-component data into a single array of counter-like data.

    Uses the big-endian convention (same as everywhere in EXA, but opposite to eg. Qiskit).
