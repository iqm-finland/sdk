...
class ChannelProperties:
    ...
    Each programmable controller in Station Control has an associated channel.
    """
...
class ProbeChannelProperties(ChannelProperties):
    ...
    # FIXME not a channel property, since it can be changed using controller settings!
...
_FAST_FEEDBACK_CHANNEL = "{probe}__feedforward_bits_to_{awg}"
_FAST_FEEDBACK_FUNCTION_AWG = "feedback_from_{probe}"
_FAST_FEEDBACK_FUNCTION_PROBE = "feedback_to_{awg}"
...
def _build_channel_properties(
    ...
    """Build channel properties from controller read-only settings received from Station Control.

    Computational resonators and qubits without a (real) drive controller get a virtual drive
    channel to store :class:`.VirtualRZ` instructions coming from two-component gates applied on
    them. These virtual channels are eventually emptied in a schedule-level pass, and are never sent
    to the station. The z rotations are applied on a neighboring qubit after the state has been
    moved there. See :class:`.MoveMarker`.

    Args:
        chip_topology: QPU topology.
        drive_controllers: Mapping from QPU component name to its drive AWG controller settings.
        flux_controllers: Mapping from QPU component name to its fast flux AWG controller settings.
        readout_controllers: Mapping from QPU component name to its readout controller settings.

    Returns:
        * Mapping from controller names to the properties of the corresponding control channels
        * Mapping from QPU component names to mapping of ``('drive', 'flux', 'readout')`` to the
          name of the controller responsible for that function for that component.

    """
    # TODO should this info come from a dedicated SC endpoint instead, so we would not have to parse SC settings:
    # EXA-1777

    VIRTUAL_DRIVE_CHANNEL = "{component}__drive_virtual"
    ...
    for function, controllers, is_iq in [("drive", drive_controllers, True), ("flux", flux_controllers, False)]:
        for component, awg_node in controllers.items():
            ...
            # fast feedback/feedforward: create feedback channels for every connected probeline/AWG pair.
            # The feedback channel has no
            if "feedback_sources" in awg_node.children:
                for source in awg_node.feedback_sources.value:
                    ...
                    channel_name = _FAST_FEEDBACK_CHANNEL.format(probe=probe_line, awg=awg_node.name)
                    ...
                    # link both the probeline and the AWG to each other and their shared feedback channel
                    # using these special component function names
                    component_to_channel[component][_FAST_FEEDBACK_FUNCTION_AWG.format(probe=probe_line)] = channel_name
                    component_to_channel[probe_line][_FAST_FEEDBACK_FUNCTION_PROBE.format(awg=awg_node.name)] = (
                        channel_name
                    )
    ...
    if awg_sampling_rates:
        ...
        for component in chip_topology.computational_resonators_sorted + tuple(qubit_components_without_drive):
            channel_name = VIRTUAL_DRIVE_CHANNEL.format(component=component)
