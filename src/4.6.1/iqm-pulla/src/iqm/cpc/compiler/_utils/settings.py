...
from exa.common.data.parameter import CollectionType, DataType, ObservationValue, Parameter, Setting, SourceType
...
def limit_components_by_settings(
    components: ComponentGrouping | Components, settings: SettingNode | None, chip_topology: ChipTopology
) -> ComponentGrouping:
...
def get_controller_settings(
    ...
    # drive settings
    override_drive_with: dict[str, ObservationValue] = {
        "status": True,
    ...
    override_flux_with: dict[str, ObservationValue] = {"awg.status": True}
    _override_operation_settings_with_values_from_dict(
        settings=settings,
        operation="flux",
        ...
        for component in components:
            for impl_name in gate_definitions[measure_gate_name].implementations:
                if component in settings.gates[measure_gate_name][impl_name].children:
                    if "acquisition_type" in settings.gates[measure_gate_name][impl_name][component].settings:
...
def _get_controller_settings(
    components: list[str], controller_mapping: dict[str, dict[str, str]], default_tree: SettingNode
) -> SettingNode:
    """Get controller settings."""
    pruned_tree = SettingNode("root", controllers=SettingNode("controllers", align_name=False))
    for component in components:
        if (controller_dict := controller_mapping.get(component)) is None:
...
def _override_operation_settings_with_values_from_dict(
    settings: SettingNode, operation: str, components: list[str], override_with: dict[str, ObservationValue]
) -> None:
    ...
    for component in components:
        for path, value in override_with.items():
            _update_setting_value(
                settings.controllers[component][operation], path.split("."), value, COMPILER_DEFAULTS_SOURCE
            )


def _update_setting_value(node: SettingNode, keys: Sequence[str], value: ObservationValue, source: SourceType) -> None:
    """Update the value of single setting in the tree, ignore if not found.

    Args:
        node: Settings node.
        keys: Settings tree path fragments.
        value: Value to set.
        source: Source for the value.

    """
    if len(keys) == 1:
        if (s := node.settings.get(keys[0])) is not None and not s.read_only:
            node[keys[0]] = value
            node[keys[0]]._source = source
        return None

    if (new_node := node.subtrees.get(keys[0])) is not None:
        _update_setting_value(new_node, keys[1:], value, source)
...
def _map_param_types(type_hint: type) -> tuple[DataType, CollectionType]:  # noqa: PLR0912
    ...
    Returns:
        Corresponding EXA :class:`.Parameter` typing information.
    ...
    if isinstance(type_hint, str):
        ...
        if len(union_args) > 2:  # noqa: PLR2004
            return DataType.ANYTHING, CollectionType.SCALAR
        type_hint = next(h for h in union_args if h != "None")
    ...
    if isinstance(type_hint, str) and "list" in type_hint:
        type_hint = type_hint.replace("list[", "").replace("]", "")
...
def assign_default_gate_implementations_from(
    ...
    """Apply the default gate implementations in the settings tree to the compiler gate definitions.

    Args:
        gate_definition_settings: The ``gate_definitions`` branch in the settings tree.
        gate_definitions: The gate definitions used by the compiler.
        skip_missing: Whether to ignore gates and implementations missing from ``gate_definitions``.

    Returns:
        Copy of ``gate_definitions`` updated with default implementations from ``gate_definitions_settings``.

    """
    # for each gate, mapping from locus to its default implementation override
    per_locus_defaults_dict: dict[str, dict[Locus, set[str]]] = {}
    for gate_name, gate_node in gate_definition_settings.child_nodes:
        if (gate_name not in gate_definitions) and skip_missing:
            continue
        quantum_op = gate_definitions[gate_name]

        if default_implementation := gate_node.default_implementation.value:
            if default_implementation in quantum_op.implementations or not skip_missing:
                set_gate_implementation_as_default(
                    gate_definitions,
                    gate_name,
                    default_implementation,
                )
        for impl_name, impl_node in gate_node.child_nodes:
            if impl_name in quantum_op.implementations or not skip_missing:
                for locus_str in impl_node.override_default_for_loci.value:
                    locus = locus_str_to_locus(locus_str)  # type: ignore[union-attr]
                    quantum_op.set_default_implementation_for_locus(impl_name, locus)
                    per_locus_defaults_dict.setdefault(gate_name, {}).setdefault(locus, set()).add(impl_name)

    # check for multiple implementations claiming the same locus
    for gate_name, gate_defaults in per_locus_defaults_dict.items():
        symmetric_locus_owner: dict[frozenset[str], str] = {}
        for locus, impls in gate_defaults.items():
            if len(impls) > 1:
                raise ValueError(
                    ...
                    f" simultaneously to `{impls}`. Make sure you don't have the same locus in"
                    " `settings.gate_definitions.<gate>.<implementation>.override_default_for_loci` for multiple "
                    f" different implementations of `{gate_name}`."
                )
            impl_name = next(iter(impls))  # just one
            # symmetric gates must not declare different default implementations for different locus orders
            if gate_definitions[gate_name].arity > 1 and gate_definitions[gate_name].symmetric:
                locus_set = frozenset(locus)
                if (first_owner := symmetric_locus_owner.get(locus_set)) is not None:
                    raise ValueError(
                        ...
                        f" has at least two: {first_owner} and {impl_name}."
                    )
                symmetric_locus_owner[locus_set] = impl_name
...
def add_default_charaterization_settings(
    ...
    _DEFAULT_CHARACTERIZATION_GATE_PROPERTIES_RECIPES: list[
        tuple[list[str], list[str] | None, list[str | None], list[Parameter]]
    ] = [
        ...
        (["cz", "move"], ["crf_crf_crf", "slepian_crf_crf"], [None], [TRIPLE_FLUX_QUBIT_TO_SWEEP]),
    ...
    _DEFAULT_CHARACTERIZATION_MODEL_RECIPES: list[tuple[list[str], list[str | None], list[Parameter]]] = [
...
def add_characterization_gate_properties_settings(
    settings: SettingNode,
    recipes: list[tuple[list[str], list[str] | None, list[str | None], list[Parameter]]],
) -> None:
...
def add_characterization_model_settings(
    ...
    components: list[str],
    chip_topology: ChipTopology,
    recipes: list[tuple[list[str], list[str | None], list[Parameter]]],
) -> None:
...
def add_pulse_settings(
    ...
    The added nodes are deduced from ``ops_table`` (that contains the quantum operation definitions),
    the chip topology, and the available control channels. The gate parameter nodes
    ...
    Args:
        ...
        station_properties: Static station properties.
    ...
    components_in_settings = frozenset(
        component
        for component in station_properties.qpu_topology.all_components
        if component in settings.controllers.children
    )
    settings.subtrees["gates"] = _create_gate_setting_nodes(
        ops_table, station_properties, limit_locus_by=components_in_settings
...
def _create_gate_setting_nodes(
    ...
    *,
    ...
    Args:
        ...
        station_properties: Static properties of the station.
    ...
    for operation_name, operation in ops_table.items():
        if limit_gates_by and operation_name not in limit_gates_by:
            continue
        o_settings: dict[str, SettingNode] = {}  # operation node
        for impl_name, impl_class in operation.implementations.items():
            if impl_name in forbidden_impls:
                continue
            oi_settings: dict[str, SettingNode] = {}  # operation.implementation subnode

            loci = list(impl_class.get_locus_mapping(station_properties))
            for locus in _get_loci(loci, limit_locus_by):
                locus_str = locus_to_locus_str(locus) if locus else GLOBAL_LOCUS_STRING_REPRESENTATION
                locus_path = path + ("gates", operation_name, impl_name, locus_str)

                # operation.implementation.locus subnode, with the default values of the GateImplementation
                # TODO iqm-pulse should not use Settings or Parameters, they should be created here.
                oil_settings = impl_class.build_setting_node(
                    locus=locus,
                    station_properties=station_properties,
                    path=locus_path,
                    use_defaults=not custom_calibration,
                )
                ...
                if issubclass(impl_class, CompositeGate) and (customizable_members := impl_class.customizable_gates):
                    # add custom calibration nodes for member gates
...
def _get_loci(
    ...
    Filter the ``loci`` such that they are limited to subsets of ``parent_locus`` components.

    Args:
        loci: Possible loci for a QuantumOp implementation.
            ...
            if max_depth is None or depth <= max_depth:
                if isinstance(node[child], SettingNode):
                    if path[0] == child:
                        ...
                        _set_for(node=node.subtrees[child], path=path, value_to_set=value_to_set, depth=depth + 1)
...
def gates_data_from(gate_calib_data: SettingNode) -> dict[str, Any]:
    ...
    """
    # TODO limit the use of this function in exa-core and avoid it entirely in exa-experiments and
    # iqm-calibration-graphs.
    ...
    def _convert_settings_recursive(node: SettingNode) -> dict[str, Any]:
        ...
        for child_name, child_node in node.child_nodes:
            converted[child_name] = _convert_settings_recursive(child_node)
        for child_name, child_setting in node.child_settings:
            converted[child_name] = child_setting.value
        return converted
...
def fix_loci(op_calib_data: dict[str, Any]) -> dict[str, Any]:
    """Convert EXA gate calibration data to ``iqm.pulse`` format.

    Convert locus strings into ``Locus`` type, simplify CompositeGate calibration nodes.

    Args:
        op_calib_data: Nested dict of gate calibration data built out of the ``gates``
            branch of the settings tree.

    Returns:
        Gate calibration data in :mod:`iqm.pulse` format.
