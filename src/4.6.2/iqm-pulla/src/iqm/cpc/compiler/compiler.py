...
from exa.common.sweep.util import NdSweep, Sweeps, convert_sweeps_to_list_of_tuples
...
class Compiler:
    ...
    def __init__(  # noqa: PLR0913
        ...
        *,
        ...
        # collect the static station properties together
        self.station_properties = StationProperties._construct(
            chip_topology,
            station_control_settings,
            self.controller_mapping,
        )

        # construct and cache locus mappings for the known gate implementations
        for quantum_op in self.gate_definitions.values():
            for impl_class in quantum_op.implementations.values():
                impl_class.get_locus_mapping(self.station_properties)
    ...
    def get_settings(  # noqa: PLR0913
        ...
        **shots** or repetitions of the executed circuit, which is easiest done using
        ...
        if circuit_stages is None:
            circuit_stages = copy(self.circuit_stages)
        if pulse_stages is None:
            pulse_stages = copy(self.pulse_stages)
        if final_stages is None:
            final_stages = copy(self.final_stages)

        data, timebox_input = validate_inputs(circuits, timeboxes)
        if timebox_input:
            stages = resolve_circuit_function_and_stages(pulse_stages, data) + final_stages
        else:
            stages = resolve_circuit_function_and_stages(circuit_stages, data) + pulse_stages + final_stages
        ...
        utils_settings.set_default_pulse_setting_values(settings.subtrees["gates"], {"rz.*": 0.0}, max_depth=4)
    ...
    def _update_settings_from_observations(self, settings: SettingNode) -> None:
        ...
        for path, obs in overrides.items():
            utils_settings._update_setting_value(settings, path.split("."), obs.value, obs)
    ...
    def run_stages(  # noqa: PLR0912,PLR0915
        ...
        """
        if stages is None:
            stages = self.circuit_stages + self.pulse_stages
        ...
        builder = self.get_schedule_builder(settings)
        context["builder"] = builder

        # split sweeps to hard and soft sweeps
        nd_sweep: NdSweep = convert_sweeps_to_list_of_tuples(sweeps) if sweeps else []
        soft_sweeps, hard_sweeps = split_to_hard_and_soft_sweeps(nd_sweep, settings)
        ...
        for spot_idx, spot in enumerate(sweep_spots):
            ...
            if "gates" in full_spot_dict:
                builder.inject_calibration(utils_settings.fix_loci(full_spot_dict["gates"]), cache=gate_impl_cache)
    ...
    def finalize(
        ...
        """
        stages = stages if stages is not None else self.final_stages
        settings = context["settings"]
        stage_spot_dict = get_stage_spot_dict(settings.stages)
        final_stages = format_stages(stages, idempotent=self.compiler_options.idempotent_stages)
    ...
    def post_process(
        ...
        """
        stages = stages if stages is not None else self.pp_stages
        cpc_logger.info("Running postprocessing stages...")
        pp_stages = format_stages(stages, idempotent=self.compiler_options.idempotent_stages)
    ...
    def get_schedule_builder(self, settings: SettingNode | None = None) -> ScheduleBuilder:
        """Get :class:`.ScheduleBuilder` for this compiler.

        Args:
            settings: Settings tree to use. If ``None``, the default settings will be used.

        Returns:
            ScheduleBuilder object.

        """
        settings = settings or self.get_settings(circuits=[])
        ...
        return ScheduleBuilder(
            ...
            # FIXME HACK override readout channel center_frequency using the settings
            self.station_properties._hack_readout_channels(settings.controllers),
        )
