...
def pack(array: np.ndarray) -> dpb_Array:
    ...
    if dtype_type == np.complex128:
        target.complex128_array.real.MergeFrom(map(np.ndarray.item, np.nditer(array.real, order="C")))  # type:ignore[arg-type]
        target.complex128_array.imag.MergeFrom(map(np.ndarray.item, np.nditer(array.imag, order="C")))  # type:ignore[arg-type]
        return target

    if dtype_type == np.bool_:
        target_field = target.bool_array
    elif dtype_type in (np.int64, np.int32):
        target_field = target.int64_array  # type: ignore[assignment]
    elif dtype_type == np.float64:
        target_field = target.float64_array  # type: ignore[assignment]
    else:
        raise ValidationError(f"Unsupported numpy array type {dtype_type} for an array.")

    target_field.items.MergeFrom(map(np.ndarray.item, np.nditer(array, order="C")))  # type:ignore[arg-type]
    return target
...
def unpack(source: dpb_Array) -> np.ndarray:
    ...
    if kind == "complex128_array":
        ...
        raise ValidationError(f"Cannot unpack value in field {kind}. Field name not recognized.")
