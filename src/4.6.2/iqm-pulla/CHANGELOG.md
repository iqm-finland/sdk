...
- Remove logger initialisation when importing `iqm.pulla.pulla`.
- Document module level type aliases as data instead of as classes, which
  autodoc could not resolve.
- Fix docstring rendering in `Compiler.compile`, `construct_run_result`,
  `ObservationHandlerBase.value_dict` and `get_conventional_control_mapping`.
- Fix the heading levels of the randomized benchmarking example notebook.
...
## Version 14.0.0 (2026-07-09)

### Breaking changes

- `iqm.cpc.compiler.compiler` imports changed.
- `iqm.cpc.compiler.compiler` imports changed.
- EXA no longer attempts to fix the loci of a locus mapping based on the
  symmetry properties of the gate or the implementation, when building
  the settings tree. It instead assumes that each `GateImplementation`
  uses the correct locus mapping.
- `FLUX_PULSED_QUBITS_2QB_MAPPING` now returns only the numerically
  sorted locus when both qubits have fast flux.
  Only the first qubit will get a flux pulse, as before.
- Heralding discarding all shots raises a PostSelectionError instead of a RuntimeError.
- The `MeasurementMode`, `CircuitBoundaryMode` and `CircuitExecutionOptions` classes
  were removed as obsolete.
- The `circuit_operations_to_cpc` function was removed as obsolete.
- Removed unused `_old_tests_archive/` directory from the package contents.

### Bug fixes

- Fix Qiskit post-processing breaking when using the reset instruction.
- Counter post-processing readout mode works correctly with MOVE-MEASURE
- Domain changed from meetiqm.com to iqm.tech
- Switch from pinned dependencies to ranges for the bare package installation.
- Allow multiple `reset` operations on the same qubit in a single
  circuit/segment in Pulla result postprocessing.

### Features

- Add a gate property for the triple fast flux CZ implementation, analogous
  to the AC Stark implementations.
- Standard EXA-style post-processing supports Move_Measure_Composite implementation.
- Fix the docstring.
- Compiler performance improved
  - No longer deepcopy settings in the beginning of compilation
    - upstream clients (EXA, SC Core API) already do this.
  - Settings to calibration data dict performance optimized
- Fixes the probe pulse being sent in circuit execution based on the
  actually used terminal measurement operation being available
- Add method `Compiler.get_schedule_builder` which initiates a ScheduleBuilder
  object with the given compiler settings (useful for e.g. debugging).
- More performance improvements especially to the standard Station Control
  circuit execution.
- Optionally always override components, even if
  the initial layout is given.
- Add optional dependency groups with pinned versions to `iqm-pulla`:
  ...
  - `qir-pin-iqm`: includes `qir` extra dependencies with inter-IQM packages
    pinned to co-released versions.
  - `qir-pin-all`: like `qir-pin-iqm`, but additionally pins all transitive
  ...
  - `qiskit-pin-all`: like `qiskit-pin-iqm`, but additionally pins all
    transitive third-party dependencies.
- Pulla `CircuitExecutionResult` now uses its own types, since the data
  differs somewhat from standard circuit execution.
- Add the `apply_drive_crosstalk` compiler pass as the last pass of the
  schedule-to-schedule stage (so the compensation pulses are not modified
  by any later pass).
  - For each given `<target>__<control>` locus, replays the control
    qubit's drive pulses on an auxiliary drive channel of the target (the
    target drive channel name with `.aux0` appended), scaled by the
    crosstalk amplitude and phase-shifted by adding the crosstalk phase to
    the `phase_increment` of the first replayed `IQPulse`. The compensation
    pulses are built by the `drive_crosstalk.drive_crosstalk` gate
    implementation.
  - Disabled by default (`drive_crosstalk_is_enabled=False`); this is not
    a full feature yet and uses placeholder example calibration values.
- CircuitCompilationResult can store settings as a dict instead of
  setting nodes, improving performance.
- All `MultiQubitExperiment` comments have been updated to `Experiment`.
- All `MultiQubitOptions` comments have been updated to `ExperimentOptions`.
- Bumps `iqm-qiskit-qir` dependency for Python 3.14 support.
