...
from exa.common.errors.iqm_error import CircuitTranspilationError, CircuitValidationError
        ...
        for idx, inst in enumerate(instructions):
            ...
            try:
                ...
                if inst.name not in self.qr_gates_q2r or any(c in self.resonators for c in locus):
                    raise CircuitTranspilationError(str(e)) from e
...
def transpile_remove_moves(circuit: Circuit) -> Circuit:
    """Convert a Star architecture circuit into an equivalent simplified architecture circuit.

    Converts a circuit involving resonators and MOVE gates into an equivalent simplified
    architecture circuit without them.
