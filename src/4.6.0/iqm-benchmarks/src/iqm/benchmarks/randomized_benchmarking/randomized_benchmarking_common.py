...
def edge_grab(
    ...
    for i, q0 in enumerate(qubit_set):
        twoq_edges.extend([[q0, q1] for q1 in qubit_set[i + 1 :] if (q0, q1) in list(backend.coupling_map)])
...
def estimate_survival_probabilities(num_qubits: int, counts: list[dict[str, int]]) -> list[float]:
    ...
    """
    return [c["0" * num_qubits] / sum(c.values()) if "0" * num_qubits in c else 0 for c in counts]
...
def survival_probabilities_parallel(
    ...
    for q in qubits_array:
        ...
        marginal_survival_probabilities[str(q)] = [
            c["0" * n_qubits] / sum(c.values()) if "0" * n_qubits in c else 0 for c in marginal_probabilities[str(q)]
        ]
