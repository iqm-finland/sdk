...
def _serialize_value(value: None | float | str | bool | dict | list) -> pb.Value:
    ...
    if value is None:
        return pb.Value(null_value=1)  # type: ignore[arg-type]
    ...
    raise ValidationError(f"Serializing of type '{type(value)}' is not supported.")
...
def _deserialize_value(proto: pb.Value) -> None | float | str | bool | dict | list | int:
    ...
    raise ValidationError(f"Unrecognized datatype field {which}")
