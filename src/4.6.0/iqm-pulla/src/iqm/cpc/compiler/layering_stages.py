...
from iqm.pulse.playlist import (
...
_FLUX_OP_NAMES = ("cz", "move", "flux_pulse")
# TODO: create attr GateImplementation.has_flux and use that instead
...
def set_layer_transformations(
    ...
    if flux_layer_transformations is not None:
        for circuit in circuits:
            for layer in circuit:
                if layer.layer_type == "flux":
                    layer.transformations = flux_layer_transformations

    if non_flux_layer_transformations is not None:
        for circuit in circuits:
            for layer in circuit:
                if layer.layer_type == "non-flux":
                    layer.transformations = non_flux_layer_transformations
...
_MAX_SCHEDULE_DURATION: int = 2**31  # about one second
"""Largest imaginable Schedule duration in samples since there is no MAXINT in Python, arbitrary."""


def _find_first_offset(
    offset_duration: int,
    flux_pulses: dict[str, list[int]],
    non_flux_pulses: dict[str, list[int]],
    pulse_type: str | None,
) -> tuple[str, int]:
    """Find the timepoint for the first instruction of the given pulse_type in the Schedule."""

    def _find_min(pulses: dict[str, list[int]]) -> int:
        min_duration = _MAX_SCHEDULE_DURATION
...
def _extract_layer(
    ...
    interval = end_duration - start_duration
    layer: dict[str, list[Instruction]] = defaultdict(list)
    first_instructions_at: dict[str, int] = {}
    for channel, segment in schedule.items():
        duration_counter = 0
        layer_instructions: list[Instruction] = []
        just_wait = True
        layer_duration = 0
        for inst in segment:
            if (
                duration_counter >= start_duration
                and duration_counter < end_duration
                and layer_duration + inst.duration <= interval
            ):
                if len(layer_instructions) == 0:
                    first_instructions_at[channel] = duration_counter
                layer_instructions.append(inst)
                layer_duration += inst.duration
                if not isinstance(inst, (Wait, Block)):
                    just_wait = False
            duration_counter += inst.duration
        if not just_wait:
            layer[channel] = layer_instructions
    # pad with wait from the start to align the layer correctly if there were "cut" Waits
    min_pre_wait = min(first_instructions_at.values())
    if len(set(first_instructions_at.values())) > 1:
        for channel, segment in layer.items():
            padding = first_instructions_at[channel] - min_pre_wait
            if padding:  # don't add pointless zero-Waits
                segment.insert(0, Wait(padding))
...
def layer_schedules(
    ...
    for schedule in schedules:
        ...
        for channel, segment in schedule.items():
            ...
            for pulse_idx, pulse in enumerate(segment._instructions):
                if "flux" in channel and isinstance(pulse, FluxPulse):
                    ...
                    "drive" in channel
                    and (
                        isinstance(pulse, (IQPulse, ConditionalInstruction))
                        or (isinstance(pulse, VirtualRZ) and pulse.standalone)
                    )
        ...
        while offset < _MAX_SCHEDULE_DURATION:
            ...
            if not layer_box.atom:
                builder.resolve_timebox(layer_box, neighborhood=0)
            layer_boxes.append(layer_box)
            layer_boxes.append(builder.barrier(builder.chip_topology.qubits_sorted)())  # type: ignore[arg-type]
            offset = next_offset
