...
def batch(y: np.ndarray, J: np.ndarray, bsize: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        ...
        * Randomly subsampled rows of J in accordance the columns selected in y_b
...
def generate_fids(d: int, length: int, m_f: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    Returns:
        ...
        * Sequence list for all combinations of fiducials seuqneces with a gate in between:
...
def outcome_probs_from_files(
    ...
    for k, filename in enumerate(datafile_names):
