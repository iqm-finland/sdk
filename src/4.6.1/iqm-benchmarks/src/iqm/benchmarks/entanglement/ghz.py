...
def fidelity_ghz_coherences(dataset: xr.Dataset, qubit_layout: list[int], num_circuits: int) -> list[Any]:
    ...
    p0 = probs_direct["0" * num_qubits] if "0" * num_qubits in probs_direct else 0
    p1 = probs_direct["1" * num_qubits] if "1" * num_qubits in probs_direct else 0
    ...
    if dataset.attrs["rem"]:
        ...
        p0_mit = probs_direct_mit["0" * num_qubits] if "0" * num_qubits in probs_direct_mit else 0
        p1_mit = probs_direct_mit["1" * num_qubits] if "1" * num_qubits in probs_direct_mit else 0
...
def get_edges(
    ...
    for edge in coupling_map:
        if edge[0] in qubit_layout and edge[1] in qubit_layout and set(edge) not in edges_patch:
