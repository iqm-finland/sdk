...
def _instruction_to_proto(
    ...
    match instruction.operation:
        ...
        case instructions.MultiplexedRealPulse():
            ...
            inst.multiplexed_real_pulse.entries.extend(real_pulse_entries)
        ...
        case instructions.MultiplexedIQPulse():
            ...
            inst.multiplexed_iq_pulse.entries.extend(iq_pulse_entries)
...
def _waveform_to_proto(waveform: waveforms.CanonicalWaveform) -> pb.Waveform:
    ...
    match waveform:
        ...
        case waveforms.CosineRiseFallDerivative():
            ret_wf.cosine_rise_fall_derivative.full_width = waveform.full_width
            ret_wf.cosine_rise_fall_derivative.center_offset = waveform.center_offset
            ret_wf.cosine_rise_fall_derivative.rise_time = waveform.rise_time

        case waveforms.ModulatedCosineRiseFall():
            ret_wf.modulated_cosine_rise_fall.full_width = waveform.full_width
            ret_wf.modulated_cosine_rise_fall.center_offset = waveform.center_offset
            ret_wf.modulated_cosine_rise_fall.rise_time = waveform.rise_time
            ret_wf.modulated_cosine_rise_fall.modulation_frequency = waveform.modulation_frequency
            ret_wf.modulated_cosine_rise_fall.phase = waveform.phase

        case waveforms.Slepian():
            ret_wf.slepian.full_width = waveform.full_width
            ret_wf.slepian.lambda_1 = waveform.lambda_1
            ret_wf.slepian.lambda_2 = waveform.lambda_2
            ret_wf.slepian.frequency_initial_normalized = waveform.frequency_initial_normalized
            ret_wf.slepian.frequency_to_minimize_normalized = waveform.frequency_to_minimize_normalized
            ret_wf.slepian.coupling_strength_normalized = waveform.coupling_strength_normalized
            ret_wf.slepian.center_offset = waveform.center_offset
            ret_wf.slepian.squid_asymmetry = waveform.squid_asymmetry
...
def _proto_to_waveform(waveform: pb.Waveform) -> waveforms.CanonicalWaveform:  # noqa: PLR0911
    ...
    match wf_type:
        ...
        case "cosine_rise_fall_derivative":
            return waveforms.CosineRiseFallDerivative(
                n_samples=waveform.n_samples,
                full_width=waveform.cosine_rise_fall_derivative.full_width,
                center_offset=waveform.cosine_rise_fall_derivative.center_offset,
                rise_time=waveform.cosine_rise_fall_derivative.rise_time,
            )

        case "modulated_cosine_rise_fall":
            return waveforms.ModulatedCosineRiseFall(
                n_samples=waveform.n_samples,
                full_width=waveform.modulated_cosine_rise_fall.full_width,
                center_offset=waveform.modulated_cosine_rise_fall.center_offset,
                rise_time=waveform.modulated_cosine_rise_fall.rise_time,
                modulation_frequency=waveform.modulated_cosine_rise_fall.modulation_frequency,
                phase=waveform.modulated_cosine_rise_fall.phase,
            )

        case "slepian":
            return waveforms.Slepian(
                n_samples=waveform.n_samples,
                full_width=waveform.slepian.full_width,
                lambda_1=waveform.slepian.lambda_1,
                lambda_2=waveform.slepian.lambda_2,
                frequency_initial_normalized=waveform.slepian.frequency_initial_normalized,
                frequency_to_minimize_normalized=waveform.slepian.frequency_to_minimize_normalized,
                coupling_strength_normalized=waveform.slepian.coupling_strength_normalized,
                center_offset=waveform.slepian.center_offset,
                squid_asymmetry=waveform.slepian.squid_asymmetry,
            )
...
def unpack_playlist(proto_playlist: pb.Playlist) -> Playlist:
    ...
    for channel in proto_playlist.channels.values():
        match channel.channel_config.WhichOneof("extended"):
            ...
            case _:
                raise ValidationError(f"Unknown channel type {channel.channel_config}.")
