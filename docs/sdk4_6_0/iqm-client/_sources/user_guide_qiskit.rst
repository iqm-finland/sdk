...
This guide illustrates the main features of the Qiskit adapter of IQM Client.
...
4. Download one of the demo notebooks from `IQM Academy <https://www.iqmacademy.com/tutorials/>`_ or the
   ...
   (Save Page As...)
5. Install the IQM Qiskit adapter as instructed below.
...
.. note::
    ...
    you should first uninstall it with ``$ uv pip uninstall qiskit-iqm``. In this case, you should also include
...
After installation, the Qiskit adapter can be imported in your Python code as follows:
...
In this section we demonstrate the practicalities of using the Qiskit adapter to execute
...
use it to retrieve an :class:`IQMBackend <iqm.qiskit_iqm.iqm_provider.IQMBackend>` instance representing
the quantum computer, and use Qiskit's :func:`~qiskit.compiler.transpile` function
followed by :meth:`IQMBackend.run <iqm.qiskit_iqm.iqm_provider.IQMBackend.run>` as usual.  ``shots`` denotes the number of times the quantum
...
.. code-block:: python
    ...
    token = "<YOUR TOKEN>"
    provider = IQMProvider(iqm_server_url, quantum_computer=quantum_computer, token=token)
...
.. note::
   ...
   keyword argument of :meth:`IQMBackend.run <iqm.qiskit_iqm.iqm_provider.IQMBackend.run>`. See also
...
:meth:`IQMBackend.run <iqm.qiskit_iqm.iqm_provider.IQMBackend.run>`, documented at :meth:`.IQMBackend.create_run_request`.
...
The calibration data for an IQM quantum computer is stored in a *calibration set*. An :class:`IQMBackend <iqm.qiskit_iqm.iqm_provider.IQMBackend>` instance
...
``calibration_set_id`` parameter to :meth:`.IQMProvider.get_backend` or :class:`IQMBackend <iqm.qiskit_iqm.iqm_provider.IQMBackend>`. If the server default
...
The :class:`IQMBackend <iqm.qiskit_iqm.iqm_provider.IQMBackend>` instance we created above provides all the standard backend functionality that one expects from a
...
parameters as :meth:`IQMBackend.run <iqm.qiskit_iqm.iqm_provider.IQMBackend.run>`.
...
Then, provide the :meth:`IQMBackend.run <iqm.qiskit_iqm.iqm_provider.IQMBackend.run>` method a mapping from the qubit indices used in the transpiled
...
much as possible.  To this end, the Qiskit adapter provides the class :class:`.IQMFakeBackend` that can
...
The Qiskit adapter contains :class:`.IQMFacadeBackend`, which allows to combine a mock remote execution with a local
