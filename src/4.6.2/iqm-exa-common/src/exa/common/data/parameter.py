...
from exa.common.errors.iqm_error import ForbiddenError, InvalidOperationError, ValidationError
...
def setting_value_to_observation(value: SettingValue) -> ObservationValue:
    ...
    if value is None:
        raise ValidationError("Tried to convert a None-valued Setting into an Observation.")
...
class DataType(IntEnum):
    ...
    def validate(self, value: Any) -> bool:
        ...
        if self in [DataType.FLOAT, DataType.NUMBER]:
        ...
        try:
            ...
            if self is DataType.BOOLEAN:
                ...
                if value_lower == "true" or value == "1":
                    return True
                if value_lower == "false" or value == "0":
                    return False
                raise InvalidOperationError(
                    f"Invalid boolean value '{value}'. Must be 'true', 'false', '0', or '1' (case-insensitive)."
                )
            if self is DataType.STRING:
                return value

            # Fallback for unhandled types (e.g., ANYTHING).
            # TODO: literal_eval is slow and potentially unsafe; replace with explicit DataType logic.
            try:
            ...
            # Re-raise Python native errors as IQM domain errors
            raise InvalidOperationError(f"Value '{value}' cannot be cast to {self.name}.") from e
...
class Parameter(BaseModel):
    ...
    def fast_construct(
        ...
        **kwargs,
        ...
        Intended for already-normalized / trusted data.
        """
    ...
    def _validate(self, value: Any, data_type: DataType | tuple[DataType, ...]) -> bool:
        ...
        if self.collection_type is CollectionType.NDARRAY:
    ...
    def build_data_array(
        ...
        if dimensions is None:
            dimensions = [self.name] if len(data.shape) == 1 else [f"{self.name}_{i}" for i in range(len(data.shape))]
        attrs = {
            "parameter": self,
        ...
        if metadata:
            ...
            if intersection:
                raise ValidationError(f"Can not have keys {intersection} in metadata dictionary")
    ...
    def create_element_parameter_for(self, indices: int | list[int]) -> Parameter:
        ...
        Raises:
            ValidationError: If ``self`` is not collection-valued.
    ...
    def update(self, value: SettingValue, source: SourceType = None) -> Setting:
        ...
        if value is None:
            if source is not None:
                raise ValidationError("Setting with no value cannot have a source.")
        elif source is None:
...
class Sweep(BaseModel):
    ...
    def validate_parameter(self) -> Self:
        if not all(self.parameter.validate(value) for value in self.data):
            raise ValidationError(f"Invalid range data {self.data} for parameter type {self.parameter.data_type}.")
