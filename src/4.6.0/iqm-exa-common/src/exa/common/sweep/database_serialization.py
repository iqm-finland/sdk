...
def decode_and_validate_sweeps(sweeps_json: str) -> Sweeps:
    ...
    Raises:
        ValidationError if decoded result is not expected return type

    """
    decoded = _loads(sweeps_json)
    if not isinstance(decoded, list):
        raise ValidationError(f"Outer type is not list type when decoding {sweeps_json}")

    for elem in decoded:
        if isinstance(elem, tuple):
            if any(not isinstance(t, Sweep) for t in elem):
                raise ValidationError("list of Sweeps must contain tuples of `Sweep` objects")
        elif not isinstance(elem, Sweep):
            raise ValidationError(f"list must contain either Sweep or Tuple elements. Got {type(elem)}")
...
def decode_return_parameters(json_str: str) -> dict[str, NdSweep | None]:
    ...
    * NdSweep and None are not converted.
    ...
    if not isinstance(decoded, list):
        raise ValidationError(f"Outer type is not list type when decoding: {json_str}")
...
class _SweepDataEncoder(json.JSONEncoder):
    ...
    def default(self, obj: Any) -> Any:
        ...
        """
        if isinstance(obj, (Sweep, Parameter)):
            obj = obj.model_dump()
