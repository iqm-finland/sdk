...
from iqm.iqm_client import CircuitCompilationOptions, IQMClient
from iqm.iqm_server_client.models import JobStatus
import numpy as np

from exa.common.errors.iqm_error import CircuitExecutionError, IQMError, OperationTimeoutError
...
class IQMSampler(cirq.work.Sampler):
    ...
    Args:
        url: URL of the IQM Server (e.g. "https://resonance.iqm.tech/"). If ``None``, must be
            provided as an environment variable IQM_SERVER_URL.
    ...
    def run_iqm_batch(self, programs: list[cirq.Circuit], repetitions: int = 1) -> list[IQMResult]:
        ...
        Raises:
            ...
            RequestTimeoutError: server did not return the results in the allocated time
    ...
    def _send_circuits(
        ...
        if status not in JobStatus.terminal_statuses():
            ...
            job.cancel()
            raise OperationTimeoutError(f"Job {job.job_id} didn't finish in the given time. Cancelled.")
