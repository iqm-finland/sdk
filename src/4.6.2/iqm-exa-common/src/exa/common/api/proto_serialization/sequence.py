...
def pack(values: Sequence) -> dpb_Sequence:
    ...
    if dtype is complex or np.issubdtype(dtype, np.complexfloating):  # dtype is complex check for contingency
        ...
        return target
    if dtype is bool:
        target_field = target.bool_array  # type: ignore[assignment]
        ...
        target_field = target.string_array  # type: ignore[assignment]
    else:
        raise ValidationError(f"Unsupported numpy array type {dtype} for a sequence.")

    target_field.items.MergeFrom(values)  # type: ignore[attr-defined]
    return target
...
def unpack(source: dpb_Sequence) -> list:
    ...
    if kind == "complex128_array":
        return [complex(real, imag) for real, imag in zip(source.complex128_array.real, source.complex128_array.imag)]
