...
4. Apply drive crosstalk compensation pulses to the schedules, if requested (disabled by default). This is the last
   schedule-level pass so the compensation pulses are not modified by any later pass.
...
from exa.common.sweep.util import NdSweep, convert_sweeps_to_list_of_tuples
...
from iqm.cpc.interface.circuit_execution import CircuitMetrics
from iqm.pulse import Circuit as Circuit_  # something weird with sphinx
...
from iqm.pulse.timebox import MeasureBox, MeasureBoxes, SchedulingStrategy, TimeBox
from iqm.station_control.interface.models import DDStrategy, MoveGateFrameTrackingMode, RunDefinition, SweepDefinition
...
def validate_settings(circuits: list[Circuit_], settings: SettingNode) -> list[Circuit_]:
...
def _get_components_that_can_be_measured(
    builder: ScheduleBuilder, context: dict[str, Any], measure_op: str
) -> set[str]:
    ...
    for component in builder.chip_topology.qubits:
        ...
        for op in (measure_op, "reset_wait"):
...
def subscribe_and_probe(  # noqa: PLR0912
    ...
    components_that_can_be_measured = _get_components_that_can_be_measured(
        builder, context, "measure_fidelity" if convert_terminal_measurements else "measure"
    )
...
def resolve_circuits(
    circuits: list[Circuit_] | list[TimeBox],
    builder: ScheduleBuilder,
    ...
    if isinstance(circuits[0], TimeBox):
        # TimeBox input
        # FIXME this pass should never be run on TimeBox input.
        return circuits  # type: ignore[return-value]
    ...
    for idx, c in enumerate(circuits):
        try:
            timeboxes.append(
                builder.circuit_to_timebox(
                    c.instructions,  # type: ignore[attr-defined]
                    name=c.name,  # type: ignore[attr-defined]
                    scheduling=scheduling_enum,
                )
            )
        except ValueError as exc:
            raise CircuitError(f"Circuit {idx}: {exc}") from exc
...
def _merge_multiplexed_timeboxes(circuit_box: TimeBox) -> TimeBox:
    ...
    # mergeable cases like for example [M(1), U(1), M(2)] -> [M(1, 2), U(1)]
    # Consider implementing this step completely differently. This seems very fragile.
    def disjoint_boxes(boxes: list[TimeBox] | None, box2: TimeBox) -> bool:
        if not boxes:
            return True
        box1 = boxes[1]  # the second box is always the MultiplexedProbeTimeBox which has the correct locus
    ...
    pending = None
    current_measure_boxes: list[MeasureBox] = []
    for gate_box in circuit_box.children:
        if isinstance(gate_box, MeasureBox):
            current_measure_boxes.append(gate_box)
            if len(current_measure_boxes) == current_measure_boxes[0].boxes_length:
                # the previous MeasureBoxes has completed
                to_be_multiplexed = MeasureBoxes(
                    current_measure_boxes,
                    measure_components=current_measure_boxes[0].measure_components,
                    additional_components=current_measure_boxes[0].additional_components,
                )
                current_measure_boxes = []
                if pending:
                    # there is a pending MeasureBoxes
                    if pending.is_multiplexable_with(to_be_multiplexed):
                        # the current MeasureBoxes do not overlap with the pending, so we multiplex the current
                        # to the pending
                        pending = pending.multiplex(to_be_multiplexed)
                    else:
                        # the current do overlap with the pending, so we must place the pending
                        placed_boxes.extend(pending)
                        pending = to_be_multiplexed
                else:
                    # there is no pending so the currently finished MeasureBox becomes the pending
                    pending = to_be_multiplexed
        elif disjoint_boxes(pending, gate_box):
            # pending and gate_box loci don't overlap, so we keep pending as is and just place gate_box
            placed_boxes.append(gate_box)
        else:
            # loci do overlap, so we must place pending first
            if pending:  # mypy wants this...
                placed_boxes.extend(pending + [gate_box])
            else:
                placed_boxes.append(gate_box)
            pending = None

    if pending:
        # finally, if there's pending, we must place that
        placed_boxes.extend(pending)
...
def multiplex_readout(timeboxes: list[TimeBox], timebox_input: bool) -> list[TimeBox]:
    """Merge any MultiplexedProbeTimeBoxes inside a TimeBox representing a circuit.
...
def prepend_heralding(
    ...
    Args:
        ...
        measure_operation: The measure operation to use, defaults to "measure". NOTE: using "measure_fidelity" is not
            recommended, since the heralding measurement should be projective to be effective, and "measure_fidelity"
            is typically not calibrated to be projective.
    ...
    components_that_can_be_measured = _get_components_that_can_be_measured(builder, context, measure_operation)
    heralded_timeboxes = []
    for circuit_idx, box in enumerate(timeboxes):
        ...
        heralded_timeboxes.append(
            builder.get_implementation(measure_operation, herald_locus)(key=HERALDING_KEY)
...
def prepend_reset(
    ...
    components_that_can_be_measured = _get_components_that_can_be_measured(builder, context, "measure")
    for circuit_idx, box in enumerate(timeboxes):
...
def apply_dd_strategy(  # noqa: PLR0913
    ...
    if "DDStrategy" in context:
        ...
        # Construct a custom DDStrategy (we assume the DDStrategy parameters are all given).
        # This way we can sweep over individual DDStrategy fields.
        gate_sequences = list(
            zip(
...
def build_playlist_and_merge_contexts(
    ...
    ignore_input_components: bool = False,
) -> Playlist:
    ...
    playlist, readout_metrics = builder.build_playlist(schedules)
    ...
    if context["inputted_components"] is None or ignore_input_components:
    ...
    context.update(dict(spot_context_values.items()))
    del context["spot_contexts"]
    return playlist


DRIVE_CROSSTALK_OP_NAME = "drive_crosstalk"
"""Name of the (optional) quantum operation that carries the drive crosstalk calibration matrix."""


def _register_aux_channel(builder: ScheduleBuilder, aux_channel: str) -> None:
    """Register an auxiliary drive channel in the builder if it is not known yet.

    The auxiliary channel physically rides on the target's drive hardware, so it reuses the target drive channel's
    properties. This registration is required because :meth:`ScheduleBuilder.build_playlist` only emits channels
    present in ``builder.channels`` and would otherwise silently drop the compensation.

    Args:
        builder: ScheduleBuilder whose channels are (possibly) extended.
        aux_channel: Auxiliary channel name of the form ``<target_drive_channel>.aux0``.

    """
    if aux_channel in builder.channels:
        return
    target_channel = aux_channel.rsplit(".aux0", 1)[0]
    target_props = builder.channels.get(target_channel)
    if target_props is not None:
        builder.channels[aux_channel] = target_props


def apply_drive_crosstalk(
    schedules: list[Schedule],
    builder: ScheduleBuilder,
    context: dict[str, Any],
    drive_crosstalk_is_enabled: bool = False,
    crosstalk_loci: list[str] | None = None,
) -> list[Schedule]:
    """Add drive-crosstalk compensation pulses to the schedules.

    For each ``<target>__<control>`` locus in ``crosstalk_loci``, the control's drive channel content is replayed on
    ``<target_drive_channel>.aux0``, IQ-scaled by the calibrated crosstalk amplitude and phase-shifted by adding the
    calibrated crosstalk phase to the ``phase_increment`` of the first replayed :class:`.IQPulse`. The compensation
    pulses are built by the ``drive_crosstalk.drive_crosstalk`` gate implementation (see :class:`.DriveCrossTalk`),
    fetched via the builder.

    This runs as the last schedule-level pass so the compensation pulses are not modified by any later pass. If
    compensation is disabled or no loci are given, the schedules are returned unchanged.

    Args:
        schedules: Schedules representing the compiled circuits.
        builder: ScheduleBuilder used to resolve channels and the compensation implementation.
        context: Compiler context (currently unused by this pass).
        drive_crosstalk_is_enabled: Enables/disables this pass.
        crosstalk_loci: ``target__control`` strings for which compensation is attempted.

    Returns:
        Schedules with auxiliary compensation channels added where possible.

    """
    if not drive_crosstalk_is_enabled or not crosstalk_loci:
        return schedules

    impl = builder.get_implementation(DRIVE_CROSSTALK_OP_NAME, ())
    for schedule in schedules:
        compensation = impl(schedule, crosstalk_loci)
        if not isinstance(compensation, TimeBox):
            raise ValueError(
                f"Drive crosstalk compensation implementation must return an atomic Timebox, got {type(compensation)}"
            )
        sched = compensation.atom

        if sched is None:
            continue

        for aux_channel, segment in sched.items():
            _register_aux_channel(builder, aux_channel)
            schedule[aux_channel] = segment
    return schedules
...
def create_run_definition(
    ...
    hard_sweeps: NdSweep = context["hard_sweeps"]
    ...
    actual_hard_sweeps = {s.parameter.name: s.data for parallel_sweep in hard_sweeps for s in parallel_sweep}
    actual_parallel_hard_sweeps = {
        parallel_sweep[0].parameter.name: [s.parameter.name for s in parallel_sweep]
        for parallel_sweep in hard_sweeps
        if len(parallel_sweep) > 1
    }
    ...
    """Maps readout labels to the hard sweeps that determine the dimensionality of their data.
    Soft sweeps are excluded because they increase the data dimensionality for all readout labels
    equally.
    """
    return_parameters, readout_components, data_parameters, ragged_labels_counts = process_return_parameters(
    ...
    # TODO: clean up the measure ancilla business once Prog RO Phase3 is done
    # the readout components should _not_ have measure ancillas, but instead the actual measured components
    flattened_readout_components = flatten_subscribed_components(readout_components)
    # the readout groups should have measure ancillas and _not_ the actual measured components
    ...
    soft_sweeps = convert_sweeps_to_list_of_tuples(context["soft_sweeps"])

    if data_size_safety_switch:
        assert_data_size_safety(_DATA_SIZE_UPPER_LIMIT, return_parameters, soft_sweeps)
    ...
    sweep_definition = SweepDefinition(
        ...
        settings=settings,
        sweeps=soft_sweeps,
        return_parameters=list(return_parameters),
        playlist=playlist,
    )
    run_definition = RunDefinition(
        ...
        additional_run_properties={
            ...
            "components_grouping_mode": components.grouping_mode.value,
        ...
        hard_sweeps=return_parameters,  # TODO the naming here is confusing
...
schedule_stage.add_passes(
    ...
    clean_schedule,
    apply_drive_crosstalk,
)
schedule_stage_experiment.add_passes(
    clean_schedule,
    apply_drive_crosstalk,
)
schedule_resolution_stage.add_passes(
    build_playlist_and_merge_contexts,
)
