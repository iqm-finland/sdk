...
if TYPE_CHECKING:
    ...
    from .gates.conditional import CCPRX_Composite
    from .gates.cz import FluxPulseGate
    from .gates.measure import Measure_CustomWaveforms
    from .gates.prx import PrxGateImplementation
...
# This is only to help typing overloads in cases where the output of "get_implementation" can be narrowed down to a
# specific class. The true list of available gates is defined in the gates module.
# Note: this only helps in the basic cases. If someone adds a funky GateImplementation with a completely different
# inheritance structure, the type hints will be wrong, but they  would be wrong also without this list.

_GateImpl_co = TypeVar("_GateImpl_co", bound=GateImplementation, covariant=True)


class _GateFactoryProtocol(Protocol[_GateImpl_co]):
    def __call__(
        self,
        locus: Sequence[str],
        ...
        *,
        ...
        priority_calibration_factorizable: OICalibrationData | None = None,
    ) -> _GateImpl_co: ...


DEFAULT_AGGREGATOR_NAME = "default_aggregator"
"""The name for the factorizable QuantumOp aggregator that uses the locus-wise default implementation for each of its
sub implementations."""
...
class ScheduleBuilder:
    ...
    Args:
        ...
        station_properties: Static station properties, e.g. QPU topology and available control channels.
    ...
    def __init__(
        ...
        self._station_properties = station_properties
    ...
    @property
    def chip_topology(self) -> ChipTopology:
        """Station QPU topology."""
        # backwards compatibility / shortcut
        return self._station_properties.qpu_topology

    @property
    def channels(self) -> dict[str, ChannelProperties]:
        """Station control channel properties."""
        # backwards compatibility / shortcut
        return self._station_properties.channels

    @property
    def component_channels(self) -> dict[str, dict[str, str]]:
        """Mapping from QPU component name to mapping from function to its controller."""
        # backwards compatibility / shortcut
        return self._station_properties.component_channels

    # See the comment about _GATE_LITERAL
    barrier: _GateFactoryProtocol[Barrier]
    prx: _GateFactoryProtocol[PrxGateImplementation]
    prx_12: _GateFactoryProtocol[PrxGateImplementation]
    cz: _GateFactoryProtocol[FluxPulseGate]
    move: _GateFactoryProtocol[FluxPulseGate]
    measure: _GateFactoryProtocol[Measure_CustomWaveforms]
    measure_fidelity: _GateFactoryProtocol[Measure_CustomWaveforms]
    cc_prx: _GateFactoryProtocol[CCPRX_Composite]

    @overload
    def __getitem__(self, item: Literal["prx", "prx_12"]) -> PrxGateImplementation: ...

    @overload
    def __getitem__(self, item: Literal["cz", "move"]) -> FluxPulseGate: ...

    @overload
    def __getitem__(self, item: Literal["measure", "measure_fidelity"]) -> Measure_CustomWaveforms: ...

    @overload
    def __getitem__(self, item: Literal["cc_prx"]) -> CCPRX_Composite: ...
    ...
    def inject_gate_definitions(self, partial_gate_definitions: dict[str, Any]) -> None:
        """Inject gate definition updates, changing :attr:`op_table` after initialisation.

        Currently only supports updating the default implementation for quantum operations.

        Args:
            partial_gate_definitions: gate definition data to be injected.

        Raises:
            ValueError: if attempting to inject unsupported QuantumOp attributes.

        """
        self.composite_cache.flush()  # always flush the composite cache if _anything_ changes in the gate impls
        for op_name, op_data in partial_gate_definitions.items():
            for op_attr, value in op_data.items():
                if op_attr == "default_implementation":
                    self.op_table[op_name].set_default_implementation(value)
                    if self.op_table[op_name].factorizable:
                        # we must clean up the factorizable cache for op if the default implementation changed
                        for locus in self._cache.get(op_name, {}).get(DEFAULT_AGGREGATOR_NAME, {}):
                            del self._cache[op_name][DEFAULT_AGGREGATOR_NAME][locus]
                else:
                    raise ValueError(
                        "Injecting QuantumOp attributes other than `QuantumOp.default_implementation` "
                        "is not supported currently."
                    )

    def inject_calibration(
        self,
        partial_calibration: OpCalibrationDataTree,
        cache: dict[str, dict[str, dict[Locus, GateImplementation]]] | None = None,
    ) -> None:
        ...
        Args:
            ...
            cache: if an implementation cache is given, we do not invalidate the cache for affected loci, but instead
                fetch the implementations from the provided cache (if they exist there).

        """
        cache = cache if cache else {}

        def _fix_cache(op: str, impl: str, locus: Locus) -> None:
            if op in cache and impl in cache[op] and locus in cache[op][impl]:
                if op not in self._cache:
                    self._cache[op] = {}
                if impl not in self._cache[op]:
                    self._cache[op][impl] = {}
                self._cache[op][impl][locus] = cache[op][impl][locus]
            elif (
                op in self._cache
        ...
        for op, op_data in partial_calibration.items():
            for impl, impl_data in op_data.items():
                for locus, locus_data in impl_data.items():
                    ...
                    _fix_cache(op, impl, locus)
                    if self.op_table[op].factorizable and op in self._cache and impl in self._cache[op]:
                        # factorizable ops only have cal data for single-component loci,
                        # but we also need to flush/update all loci that include the single-component locus
                        # for that particular factorizable QuantumOp, since one can
                        # mix single-component implementations in a multi-component factorizable implementation
                        locus_component = locus[0]
                        # dict size cannot change while you iterate over it, hence the list of keys
                        for cached_impl in self._cache[op]:
                            # flush the default aggregator and the chosen impl
                            if cached_impl in (impl, DEFAULT_AGGREGATOR_NAME):
                                for cached_locus in list(self._cache[op][cached_impl]):
                                    if locus_component in set(cached_locus):
                                        _fix_cache(op, cached_impl, cached_locus)

    def get_cache(
        self,
        ops: Iterable[str] | None = None,
        implementations: Iterable[str] | None = None,
        loci: Iterable[tuple[str, ...]] | None = None,
    ) -> dict[str, dict[str, dict[Locus, GateImplementation]]]:
        """Get a copy of the gate implementation cache.

        Args:
            ops: limit to these QuantumOps.
            implementations: limit to these GateImplementations.
            loci: limit to these loci.
        ...
        return {
            op: {
                impl: {loc: impls for loc, impls in self._cache[op][impl].items() if (loci is None or loc in loci)}
                for impl in self._cache[op]
    ...
    def get_probe_channel(self, component: str) -> str:
        ...
        """
        if component in self.chip_topology.probe_lines:
            return self._get_channel_for_component(component, "readout")
    ...
    def get_virtual_feedback_channel_for(self, awg_name: str, feedback_qubit: str) -> str:
        ...
        Args:
            awg_name: Name of the AWG controller that receives the feedback bit.
        ...
        if probe_line is None:
            raise ValueError(f"Component {feedback_qubit} has no probe line.")
        ...
        if not channel:
            raise ValueError(
                f"AWG node {awg_name} does not support fast feedback from "
                f"{probe_line} (probe line of {feedback_qubit})."
            )
        return channel
    ...
    def has_calibration(
        ...
        """True iff there is calibration data for the given quantum operation, implementation and locus.
    ...
    def get_calibration(
        ...
        *,
        ...
        Args:
            ...
            impl_name: name of the implementation
            locus: locus of the operation
            strict: Iff True, raise an error if the calibration data is missing.
    ...
    # Overloads: for explanation, see comment below _GATE_LITERAL

    @overload
    def get_implementation(
        self,
        op_name: Literal["prx", "prx_12"],
        locus: Sequence[str],
        impl_name: str | None = None,
        *,
        strict_locus: bool = False,
    ...
    def get_implementation(
        self,
        op_name: Literal["measure", "measure_fidelity"],
        locus: Sequence[str],
        impl_name: str | None = None,
        *,
        strict_locus: bool = False,
    ...
    def get_implementation(
        ...
        *,
        ...
        For convenience, the returned implementation may use a permuted ``locus`` (and the calibration data
        associated with it) based on the following rules:

        * If the operation is not symmetric (e.g. CX), different ``locus`` orders correspond to different
          operations, and thus the locus order may not be changed.

        * If the operation is symmetric (e.g. CZ), all ``locus`` orders correspond to the same logical operation.
          However, symmetric operations may have non-symmetric implementations.

          * If the chosen implementation is symmetric, we order ``locus`` numerically (not lexicographically!),
            so that we only need to have calibration data for one locus order.

          * Otherwise, if ``strict_locus`` is False, we check all permutations of ``locus``, and pick the first one that
            has calibration data available. The original order is checked first.
            If ``strict_locus`` is True, only the original order may be used.

        The attributes :attr:`QuantumOp.factorizable`, :attr:`GateImplementation.needs_calibration` and whether
        the implementation is a :class:`CompositeGate` interact in a nontrivial way.
        Examples of their currently used combinations:
        ...
        Args:
            ...
            impl_name: Name of the implementation. ``None`` means the implementation is chosen automatically
                using :meth:`.QuantumOp.get_default_implementation_for_locus`.
            strict_locus: Only has an effect if the operation is symmetric (e.g. CZ).
                Iff False and the chosen *implementation* is non-symmetric, the locus order may
                be changed if no calibration data is available for the requested locus order.
            priority_calibration: Single-locus calibration data to override the common calibration
                data in :attr:`calibration`. Any non-None values found in ``priority_calibration``
                will be merged to the common calibration.
                If used, ``impl_name`` must not be None since ``priority_calibration`` is implementation-specific.
                Note: using ``priority_calibration`` will prevent caching.
            priority_calibration_factorizable: Like ``priority_calibration``, but for factorizable QuantumOps.
                Mapping from single-qubit loci to overrides of their calibration data.
        ...
        impl_name = impl_name if impl_name != DEFAULT_AGGREGATOR_NAME else None
        locus = tuple(locus)
        impl_name_given = impl_name
        impl_name, locus = self._pick_implementation_and_locus(op, impl_name, locus, strict_locus=strict_locus)

        if op.factorizable:
            priority_calibration_factorizable = priority_calibration_factorizable or {}
            if len(locus) > 1:
                # Return an aggregator. Factorizable gate implementations must be capable of aggregating their
                # single-qubit sub-implementations.
                impl_class = op.aggregator_class

                def aggregator_callback() -> GateImplementation:
                    return impl_class.construct_aggregator(  # type: ignore[union-attr]
                        parent=op,
                        name=impl_name,
                        locus=locus,
                        sub_implementations={
                            c: self.get_implementation(
                                op_name,
                                (c,),
                                impl_name_given,  # if None, the sub-impls can each pick a different implementation
                                priority_calibration=priority_calibration_factorizable.get((c,)),
                            )
                            for c in locus
                        },
                        builder=self,
                ...
                if priority_calibration_factorizable:
                    # cannot be cached
                    return aggregator_callback()
                return self._cached_implementation(op_name, impl_name, locus, aggregator_callback)

            elif priority_calibration_factorizable:
                priority_calibration = priority_calibration_factorizable[locus]
                # Return a sub-implementation.

        def callback() -> GateImplementation:
            return self._build_implementation(op, impl_name, locus, priority_calibration=priority_calibration)

        if priority_calibration:
            # cannot be cached
            return callback()
        return self._cached_implementation(op_name, impl_name, locus, callback)

    def _pick_implementation_and_locus(
        self,
        op: QuantumOp,
        impl_name: str | None,
        locus: Locus,
        *,
        ...
        """Find a valid implementation and locus for the given quantum operation.

        For symmetric ``op``, may change the locus order (since it corresponds to the same logical operation).

        Args:
            op: Quantum operation.
            impl_name: Name of the requested implementation. ``None`` means use the default implementation.
        ...
        Raises:
            ValueError: Requested operation/implementation/locus not possible.

        """

        def _validate_impl_name(impl_name: str) -> type[GateImplementation]:
            if (impl_class := op.implementations.get(impl_name)) is None:
                raise ValueError(f"Unknown quantum operation implementation '{op.name}.{impl_name}'.")
            return impl_class

        if op.factorizable and len(locus) > 1:
            # For factorizable gates and factorizable loci we just return the locus as is
            # (we cannot really validate consistently either) as the validity of the locus depends then on the validity
            # of the single-component sub-loci (these are checked when creating the aggregator).
            # The implementation name is either already pinned by the user or if not, we return a generic
            # DEFAULT_AGGREGATOR_NAME denoting an aggregator that picks the default implementation for each
            # of the member components
            if impl_name is not None:
                # only validate the factorizable impl class if the implementation already pinned by user,
                # otherwise the operation's aggregator class will be used
                _validate_impl_name(impl_name)
            # normalize factorizable locus for symmetric ops
            factorizable_locus = tuple(sort_components(locus)) if op.symmetric else locus
            return impl_name or DEFAULT_AGGREGATOR_NAME, factorizable_locus
        # otherwise, pick implementation
        if impl_name is None:
            impl_name = op.get_default_implementation_for_locus(locus)
        impl_class = _validate_impl_name(impl_name)

        def modify_locus(given_locus: Locus) -> Locus:
            """See if the locus should be permuted."""
            if not op.symmetric:
                # For non-symmetric ops the locus cannot be changed.
                return given_locus
            # all locus orders are equivalent for symmetric ops, locus may be permuted
            if impl_class.symmetric:
                # Cal data for symmetric implementations uses always a sorted locus order.
                return tuple(sort_components(given_locus))
            # Symmetric op, nonsymmetric implementation (e.g. fast flux CZ):
            if strict_locus:
                # The locus order can be meaningful in practice, since multiple locus orders may have
                # separate, independent cal data.
                return given_locus
            # User did not request a strict locus order, so pick the first one that has cal data,
            # preferring the given locus. TODO maybe this is not that helpful?
            if not impl_class.needs_calibration():
                return given_locus
            for new_locus in itertools.permutations(given_locus):
                if self.has_calibration(op.name, impl_name, new_locus):
                    return new_locus
            raise ValueError(f"No calibration data for '{op.name}.{impl_name}' at {given_locus}.")

        new_locus = modify_locus(locus)

        # Check locus validity (fail early, we would fail anyway when trying to find cal data if the locus is invalid)
        valid_loci = impl_class.get_locus_mapping(self._station_properties)
        if op.arity == 0:
            # arity-0 gates use locus mappings that consist of single-component loci
            if all((c,) in valid_loci for c in new_locus):
                return impl_name, new_locus
        elif new_locus in valid_loci:
            return impl_name, new_locus
        raise ValueError(f"{op.name}.{impl_name}: {locus} is not a valid locus.")

    def _cached_implementation(
        self,
        op_name: str,
        impl_name: str,
        locus: Locus,
        callback: Callable[[], GateImplementation],
    ) -> GateImplementation:
        """Return a cached GateImplementation, or build one using ``callback`` if not found."""
        # use a cached factory if it exists
        op_cache = self._cache.setdefault(op_name, {})
        impl_cache = op_cache.setdefault(impl_name, {})
        if factory := impl_cache.get(locus):
            return factory

        # build and cache the factory
        factory = callback()
        impl_cache[locus] = factory
        return factory
    ...
    def _build_implementation(
        ...
        *,
        ...
        Args:
            ...
            impl_name: Name of the implementation.
            locus: Locus of the operation.
            priority_calibration: Calibration data to override the common calibration data.
                Only overrides the given parameters.
                Note: using ``priority_calibration`` will prevent caching.
        ...
        """
        # find the calibration data, construct the factory
        impl_class = op.implementations[impl_name]
        if not impl_class.may_have_calibration():
            # simplest case: no cal data is ever needed, e.g. rz.virtual
            factory = impl_class(parent=op, name=impl_name, locus=locus, calibration_data={}, builder=self)
        else:
            # Either needs_calibration or is CompositeGate with customizable members.
            # Find the calibration data, which is all found under locus.
            cal_data = self.get_calibration(op.name, impl_name, locus, strict=impl_class.needs_calibration())
            if priority_calibration:
                cal_data = merge_dicts(cal_data, priority_calibration, merge_nones=False)
            validate_locus_calibration(cal_data, impl_class, op, impl_name, locus)
    ...
    def get_implementation_class(self, op_name: str, impl_name: str | None = None) -> type[GateImplementation]:
        ...
        Args:
            op_name: Name of the quantum operation.
            impl_name: Name of the implementation (``None`` means use the global default implementation).
        ...
        if impl_name is None:
            impl_name = op.default_implementation
    ...
    def circuit_to_timebox(
        ...
        *,
        ...
        Args:
            ...
            scheduling: scheduling strategy (e.g. ASAP or ALAP) to be used in resolving the TimeBoxes.
        ...
        for op in circuit:
            ...
            factory = self.get_implementation(op.name, mapped_locus, op.implementation)
            boxes.append(factory(**op.args))
        return TimeBox.composite(boxes, label=name, scheduling=scheduling, scheduling_algorithm=scheduling_algorithm)
    ...
    def _fuse_long_distance_virtual_rzs(self, schedule: Schedule) -> Schedule:
        """Fuse out-of-gate-locus VirtualRZs with the next IQPulse after the FluxPulse they are correcting."""
    ...
    def _resolve_timebox_hard_boundary(
        ...
        """Resolve a TimeBox using the ``HARD_BOUNDARY`` algorithm.

        Treats each composite TimeBox as a solid rectangle (the longest channel within defines the duration).
        """
    ...
    def _get_neighborhood_hard_boundary(self, box: TimeBox, neighborhood: int) -> set[str]:
        ...
        Returns:
            QPU components (plus maybe channels?) belonging the given neighborhood of ``box``.
    ...
    def _resolve_timebox_tetris(
        ...
        *,
        ...
        """Resolves a TimeBox using the ``TETRIS`` algorithm.

        Packs the schedule as tightly as possible
        (solid instructions still cannot overlap) regardless of the TimeBox boundaries.
        """
