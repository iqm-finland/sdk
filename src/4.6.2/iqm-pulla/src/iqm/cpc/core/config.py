...
class ComponentGrouping(list):
    ...
    def copy(self) -> ComponentGrouping:
    ...
    def contains(self, elem: str | tuple[str, ...] | list[tuple[str, ...]]) -> bool:
        ...
        The behaviour depends on the :attr:`grouping_mode`` and on the type of the searched element.
    ...
    @staticmethod
    def _validate_flat(flat_components: Sequence) -> None:
        """All items must be str."""
        if sum(isinstance(c, str) for c in flat_components) != len(flat_components):
            raise MALFORMED_COMPONENTS_ERROR

    def _validate_groups(self, groups: Iterable) -> None:
        for group in groups:
            if not isinstance(group, tuple):
                raise MALFORMED_COMPONENTS_ERROR
            self._validate_flat(group)

    def _validate_colour_groups(self, colour_groups: list) -> None:
...
def _import_from_file(name: str) -> type[GateImplementation]:
    ...
    raise ValueError(
        f"The custom provided implementation {name} can't be imported. "
        "Use a path of the form ``/path/to/file.py::Class``"
    )
...
def get_conventional_control_mapping(
    ...
    extend the :attr:`.ExperimentConfiguration.controllers` and
    :attr:`.ExperimentConfiguration.component_groups` based on the convention.
...
class ExperimentConfiguration:
    ...
    * Any other top-level keys generate :class:`ExperimentDefaultsConfiguration` instances for specific
    ...
    def register_gate_implementation(
        self,
        gate_name: str,
        impl_name: str,
