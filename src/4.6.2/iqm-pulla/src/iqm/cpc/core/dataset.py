...
def apply_along_coordinate(  # noqa: PLR0915
    ...
    The main use case for this is fitting, where ``func`` returns a fit to some raw data,
    plus the found fit parameters.

    For example, if ``data_array`` has dimensions A, B, and C, and we apply a fitting function along
    B, the fitter ``func`` function is evaluated for each coordinate tuple spanned by A and C and
    gets arrays of len(B) as input.
    ...
    Args:
        ...
        prefix: If given, the names of all arrays in ``returns`` are prefixed with ``f"{prefix}_"``.
            Used to avoid name clashes with existing data variables.

    Returns:
        Data arrays depending on ``returns``. The first array is always the result produced by applying
...
def convert_2d_model(
    ...
    Args:
        model: Original function taking the x, y independent variables, and variadic args.
        cloned_arguments_indices: Indices of the arguments we would like to clone in the list of all the arguments
    ...
    def _model(data: np.ndarray, *args) -> np.ndarray:
        x, y, y_ind = data
        ...
        for i in range(count):
            if i in cloned_arguments_indices:
                ...
                next_arg = args[j]
                j += 1
            red_args.append(next_arg)
        return model(x, y, *red_args)

    return _model


def _get_indices(model: Callable, cloned_arguments: None | Iterable[str], required_estimates: list[str]) -> list[int]:
    """Convert the names of cloned arguments into indices in the function signature corresponding to those.

    Args:
        model: The function to use for fitting.
        cloned_arguments: Names of parameters of ``model`` which should be fitted separately for each y value.
        required_estimates: Names of parameters of ``model`` that must be provided in estimates.

    Returns:
        Indices of the ``required_estimates`` parameters of ``model`` included in ``cloned_arguments``.
...
def _prepare_data_arrays(
    ...
    """For each returned quantity, prepare an empty data array to later contain the fit results.

    The shape of this array will depend on whether the argument corresponding to the quantity is cloned.
    """
    collected = []
...
def _multifit_at_spot(  # noqa: ANN202, PLR0913
    ...
    """Perform the fit with ``func`` at each previously prepared spot, assign the results to arrays prepared beforehand.

    Note that the data arrays inside ``collected`` and thus this list itself, is modified.
    """
    fit_parameters = {}
