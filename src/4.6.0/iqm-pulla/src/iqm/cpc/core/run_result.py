...
def _crop_incomplete_data(
    ...
    for parameter, data, hard_sweeps in incomplete_data_variables:
        ...
        #              |
        #              +--- incomplete
        incomplete_dim = np.argmax(iterations_per_complete_dim > successful_iterations).item()
...
def _crop_sweep(complete: int, index: int, sweep_tuple: ParallelSweep, incomplete_dim: int) -> ParallelSweep:
...
def _annotate(array: xr.Variable, parameter: Parameter) -> None:
    ...
    if parts[0] == "gates" and len(parts) > 3:  # noqa: PLR2004
