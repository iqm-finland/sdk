...
def _gate_inst_to_str(inst: Call) -> CircuitOperation | None:
    """Convert a QIS instruction to a CircuitOperation.

    Args:
        inst: QIS instruction to convert.

    Returns:
        The CircuitOperation corresponding to ``inst``, or None if ``inst`` is not a quantum instruction.

    Raises:
        ValueError: ``inst`` is not supported.
        ValueError: ``inst`` has an invalid number of arguments.
...
def _find_arg_by_type(args: Iterable[Value], type_check_func: Callable[[Type], bool]) -> Value:
    for arg in args:
...
def _find_args_by_type(args: Iterable[Value], type_check_func: Callable[[Type], bool]) -> list[Value]:
    matches = [arg for arg in args if type_check_func(arg.type)]
    return matches


def _find_double_args(args: Iterable[Value]) -> list[Value]:
    """Return non-qubit, non-result arguments which should be doubles."""
    return [arg for arg in args if arg.type.is_double]


def _parse_double(value: Value) -> float:
...
def qir_to_pulla(  # noqa: PLR0915, PLR0912
    ...
    Args:
        ...
        qir: QIR source or bitcode to convert to a circuit.
        qubit_mapping: Dictionary mapping QIR qubit indexes to physical qubit names,
                       None will assume opaque pointers match physical names.

    Returns:
        Quantum circuits extracted from the QIR module, compiler with updated ``component_mapping``.
    ...
    circuits = [Circuit(name=name, instructions=circuit_instructions)]
...
def generate_qiskit_qir_qubit_mapping(qiskit_circuit: QuantumCircuit, qiskit_backend: BackendV2) -> QubitMapping:
    """Fix the qiskit-qir qubit pointer bug.
