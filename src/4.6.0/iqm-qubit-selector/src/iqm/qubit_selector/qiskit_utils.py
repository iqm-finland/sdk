...
def deflate_circuit(input_circ: QuantumCircuit) -> QuantumCircuit:
    ...
    active_qubit_map = {
        val: idx for idx, val in enumerate(sorted(active_qubits, key=lambda x: input_circ.find_bit(x).index))
    }
    active_bit_map = {
        val: idx for idx, val in enumerate(sorted(active_clbits, key=lambda x: input_circ.find_bit(x).index))
    }
