...
def iqm_circuit_to_gate_implementation(circuit: Circuit, qubit_mapping: QubitMapping) -> type[CompositeGate]:
...
def map_qubit_indices(circuits: Iterable[Circuit], context: dict[str, Any]) -> tuple[list[Circuit], dict[str, Any]]:
...
def get_hash_for(circuit: Circuit) -> int:
    """Get a unique id hash for a given circuit.

    In the context of this function, two circuits are considered equal if they have:
...
def _update_channel_props_from_calibration(
    ...
    Returns:
        updated ``channel_properties``

    """
    # TODO SW-1108: get rid of this and find a better way of passing center_frequency to measure gate implementation
    replacements = {}
