...
from exa.common.errors.iqm_error import ConflictError, NotFoundError, UnknownObservationError, ValidationError
...
_PATTERN: Final[str] = r"^[A-Za-z0-9_]+$"
...
class QON:
    ...
    def from_str(cls, name: str) -> QON:
        ...
        Raises:
            ValidationError: Failed to parse ``name`` because it was syntactically incorrect.
        ...
        if len(parts) < 3:
            raise ValidationError("Unparseable observation name.")
        ...
        match parts[0]:
            ...
            case Domain.CHARACTERIZATION if parts[1] == "other":
                return QONOther._parse(parts[2])  # user-defined namespace
...
class QONOther(QON):
    ...
    def __str__(self) -> str:
        return _FIELD_SEPARATOR.join([self.domain, "other", self.rest])

    @classmethod
    def _parse(cls, rest: str) -> QONOther:
        """Parse a user-defined observation name."""
        return cls(rest=rest)
...
class QONGateDefinition(QON):
    ...
    def _parse(cls, gate: str, rest: str) -> QONGateDefinition:
        ...
        if len(parts) > 2:
            raise ValidationError("gate_definitions observation name has more than 3 parts")
...
class QONCharacterization(QON):
    ...
    def _parse(cls, rest: str) -> QONCharacterization:
        ...
        parts = rest.split(_FIELD_SEPARATOR, maxsplit=1)
        if len(parts) < 2:
            raise ValidationError("characterization.model observation name has less than 4 parts")
...
class QONGateCharacterization(LocusQON):
    ...
    def _parse(cls, rest: str) -> QONGateCharacterization:
        ...
        if len(parts) < 4:
            raise ValidationError("characterization.gate_properties observation name has less than 6 parts")
        ...
        def decorator(subclass: type[QONMetric]):  # noqa: ANN202
            for name in method_names:
                if (owner := cls._registry.get(name)) is not None:
                    raise ConflictError(f"Method {name} already registered to {owner.__name__}")
...
class QONGateMetric(QONMetric):
    ...
    def _parse(cls, method: str, method_specific_part: str) -> QONGateMetric:
        ...
        Raises:
            ValidationError: Observation name is malformed.

        """
        parts, suffixes = _split_obs_name(method_specific_part, maxsplit=3)
        if len(parts) < 4:
            raise ValidationError(f"{method} gate quality metric name has less than 6 parts")
...
class QONSystemMetric(QONMetric):
    ...
    def _parse(cls, method: str, method_specific_part: str) -> QONSystemMetric:
        ...
        if len(parts) < 2:
            raise ValidationError(f"{method} system quality metric name has less than 4 parts")
...
class QONControllerSetting(QON):
    ...
    component: Annotated[str, Field(pattern=_QPU_COMPONENT_PATTERN)]
    """Name of QPU component that the controller controls."""

    controller_path: tuple[str, ...]
    """Names of the controllers and subcontrollers.

    NOTE: The first controller path element is not the same as in Station Control, where it
    includes :attr:`component`.
    """
    ...
    def __str__(self) -> str:
        return _FIELD_SEPARATOR.join([self.domain, self.component, *self.controller_path, self.setting])

    @classmethod
    def _parse(cls, component: str, rest: str) -> QONControllerSetting:
        """Parse a controller setting observation name."""
        parts = rest.split(_FIELD_SEPARATOR, maxsplit=5)  # a finite number of splits should be enough
        return cls(
            component=component,
            controller_path=tuple(parts[:-1]),
            setting=parts[-1],
        )
...
class QONGateParam(LocusQON):
    ...
    def _parse(cls, gate: str, rest: str) -> QONGateParam:
        ...
        if len(parts) < 3:
            raise ValidationError("Gate parameter observation name has less than 5 parts")
...
def _convert_to_float(value: Any) -> float:
    ...
    Raises:
        ValidationError: Conversion not possible.

    """
    try:
        return float(value)  # type: ignore[arg-type]
    except (ValueError, TypeError) as e:
        raise ValidationError(f"Cannot convert value to float: {type(value)}") from e
...
class ObservationFinder(dict):
    ...
    def __init__(self, observations: Iterable[ObservationBase], skip_unparseable: bool = False):
        def parse_observation_into_dict(name: str, dictionary: dict[str, Any]) -> tuple[dict[str, Any], str, Suffixes]:
            ...
            match path[0]:
                case Domain.METRIC:
                    if len(path) < 4:
                        raise ValidationError("Quality metric observation name has less than 4 parts.")
                case Domain.CHARACTERIZATION:
                    if len(path) < 4:
                        raise ValidationError("Characterization observation name has less than 4 parts.")
                case Domain.CONTROLLER_SETTING:
                    if len(path) < 3:
                        raise ValidationError("Controller setting observation name has less than 3 parts.")
                case Domain.GATE_PARAMETER:
                    if len(path) < 5:
                        raise ValidationError("Gate parameter observation name has less than 5 parts.")
                case Domain.GATE_DEFINITION:
                    if len(path) < 3:
                        raise ValidationError("Gate definition observation name has less than 3 parts.")
                case _:
                    raise UnknownObservationError("Unknown observation domain.")
        ...
        for obs in observations:
            ...
            except (ValidationError, UnknownObservationError) as err:
    ...
    def _build_dict(self, pre_path: Iterable[str], keys: Iterable[str], post_path: Iterable[str]) -> dict[str, float]:
        ...
        Raises:
            NotFoundError: Could not follow ``pre_path``.
        ...
        for step in pre_path:
            ...
            if not next_node or not isinstance(next_node, dict):
                raise NotFoundError(f"pre_path step {step} could not be found")
            base_node = next_node
    ...
    def _follow_path(self, path: Iterable[str]) -> dict[str, Any] | ObservationBase:
        ...
        for step in path:
            if isinstance(node, ObservationBase):
                raise NotFoundError(f"path step '{step}' could not be found")
    ...
    def _get_path_value(self, path: Iterable[str]) -> float:
        ...
        if not isinstance(node, ObservationBase):
            raise ValidationError(f"path {path} does not end in an observation")
    ...
    def _get_path_node(self, path: Iterable[str]) -> dict[str, Any]:
        ...
        node = self._follow_path(path)
        if isinstance(node, ObservationBase):
            raise ValidationError(f"path {path} does not end in a node")
        return node
