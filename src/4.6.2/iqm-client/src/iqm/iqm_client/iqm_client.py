...
class IQMClient:
    ...
    def quantum_computer_name(self) -> str:
        ...
        name = self._iqm_server_client._quantum_computer
        if name == "default":
            quantum_computers = self._iqm_server_client.get_quantum_computers()
            name = next(qc for qc in quantum_computers if qc.alias == name).display_name
        return name
